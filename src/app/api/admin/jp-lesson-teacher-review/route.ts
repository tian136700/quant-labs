...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (url.searchParams.get("summary") === "1") {
      const summaries = await listJpLessonTeacherReviewSummaries(env.DB);
    ...
    const sort = SORT_FIELDS.has(sortRaw as JpLessonTeacherReviewSortField)
      ? (sortRaw as JpLessonTeacherReviewSortField)
      : "updated_at";
    const order = orderRaw === "asc" ? "asc" : "desc";

    const data = await listJpLessonTeacherReviews(
      env.DB,
      teacherId,
      sort,
      order,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await saveJpLessonTeacherReview(env.DB, {
      id,
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const { deleted } = await deleteJpLessonTeacherReviewRecords(env.DB, ids);
