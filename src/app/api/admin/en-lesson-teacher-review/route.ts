...
import {
  deleteEnLessonTeacherReviewRecords,
  listEnLessonTeacherReviewSummaries,
  listEnLessonTeacherReviews,
  saveEnLessonTeacherReview,
} from "@/lib/en-lesson-teacher-review-db";
import type { EnLessonTeacherReviewSortField } from "@/lib/types";

const SORT_FIELDS = new Set<EnLessonTeacherReviewSortField>([
  "class_date",
  "score",
  "updated_at",
]);
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (url.searchParams.get("summary") === "1") {
      const summaries = await listEnLessonTeacherReviewSummaries(env.DB);
    ...
    const sort = SORT_FIELDS.has(sortRaw as EnLessonTeacherReviewSortField)
      ? (sortRaw as EnLessonTeacherReviewSortField)
      : "updated_at";
    const order = orderRaw === "asc" ? "asc" : "desc";

    const data = await listEnLessonTeacherReviews(
      env.DB,
      teacherId,
      sort,
      order,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await saveEnLessonTeacherReview(env.DB, {
      id,
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const { deleted } = await deleteEnLessonTeacherReviewRecords(env.DB, ids);
