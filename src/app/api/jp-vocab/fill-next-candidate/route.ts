...
import { pickNextJpVocabFillCandidate } from "@/lib/jp-vocab-fill-next-candidate";
...
export async function POST(request: Request) {
  try {
    ...
    const limited = await enforceVocabFillRouteRateLimit(
      env.DB,
      request,
      "/api/jp-vocab/fill-next-candidate"
    );
    ...
    const candidate = await pickNextJpVocabFillCandidate(env.DB);
    return jsonResponse({
      ok: true,
      mode: "next_candidate",
      candidate,
