import {
  getD1QuotaDiagnosticSummary,
  purgeD1QuotaSignalsOlderThan,
} from "@/lib/d1-quota-db";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const summary = await getD1QuotaDiagnosticSummary(env.DB, {
      quotaStatDate,
    });

    void purgeD1QuotaSignalsOlderThan(env.DB).catch(() => {});
