...
import { syncEnLessonTitleByRefKey } from "@/lib/en-lesson-db";
import { getEnVocabRef, saveEnVocabRefFileMeta } from "@/lib/en-vocab-db";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (title !== existing.title) {
      await syncEnLessonTitleByRefKey(env.DB, refKey, title);
    }
    ...
    return jsonResponse({
      ...
      view_path: `/api/en-vocab/ref/${refKey}`,
    });
  } catch (err) {
