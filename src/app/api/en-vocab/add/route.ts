...
import {
  addEnVocabWord,
  getEnVocabRef,
  getOrUploadEnVocabRefByContent,
...
import type { EnVocabKind, EnVocabMediaType } from "@/lib/types";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const existingRefKey = normalizeEnVocabRefKey(
    ...
    if (file instanceof File && file.size > 0) {
      ...
      const existing = await getEnVocabRef(env.DB, refKey);
      if (!existing) {
        return jsonResponse(
    ...
    const result = await addEnVocabWord(env.DB, {
      word,
      reading,
      meaning,
      kind,
    ...
    return jsonResponse({
      ...
      ref_view_path: refKey ? `/api/en-vocab/ref/${refKey}` : null,
    });
  } catch (err) {
