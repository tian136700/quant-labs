...
import {
  clearEnVocabReviewDone,
  recordEnVocabReviewDone,
  getEnVocabReviewProgress,
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const review_progress = await getEnVocabReviewProgress(env.DB);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (body.action === "review_next") {
      ...
      const review_progress = await recordEnVocabReviewDone(env.DB, wordId);
      return jsonResponse({ ok: true, review_progress });
    }

    if (body.action === "clear") {
      const review_progress = await clearEnVocabReviewDone(env.DB);
      return jsonResponse({ ok: true, review_progress });
    }
