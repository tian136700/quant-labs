...
const TEACHER_SELECT = `SELECT id, name, sort_order, created_at, updated_at FROM en_lesson_teacher`;
...
export async function updateEnLessonTeacher(
  ...
  try {
    const result = await db
      .prepare(
        `UPDATE en_lesson_teacher SET name = ?1, sort_order = ?2, updated_at = ?3 WHERE id = ?4`
      )
