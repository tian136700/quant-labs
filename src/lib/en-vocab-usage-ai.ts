...
export const EN_VOCAB_USAGE_UPLOAD_SPEC = {
  version: 3,
  count_rule:
    "组数=该词真实不同核心义项数（按分类语境选题，托业偏职场商务）；只有 1 种就 1 条，有几种写几条；禁止按对象/场景硬拆同一义",
  format_example:
...
- ❌ 禁止把同一义项的近义改写、语气微调、缩小场景、换主语/宾语当两条（如都在解释同一个「有吸引力」、同一个「仔细地」，或同一个介词「在……之中」）。
- 自检：若两条候选用法造出的例句几乎可互换（都是 carefully 读合同 / carefully 核对数），说明仍是同一核心义 → 必须合并成 1 条，只留 1 条例句。
- ✅ 只有核心意思真不同时才拆：不同词性（介词 vs 副词）、不同词典义（issue「问题」vs「发行」）、或不同固定结构且意思不同（expect that vs be expected to；would rather vs rather = 相当）。
