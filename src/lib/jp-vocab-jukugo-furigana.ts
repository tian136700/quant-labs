 ...
 * 数量词训读见 `jp-vocab-quantity-furigana.ts`（〜つ／人／日／歳／几点）。
 * apply / 线上 normalize 拒 wrong_jukugo_furigana。
 */

import {
  JP_VOCAB_QUANTITY_FURIGANA_PROMPT_HINT,
  jpVocabExampleHasWrongQuantityFurigana,
} from "@/lib/jp-vocab-quantity-furigana";

export {
  JP_VOCAB_AGE_FULL,
  JP_VOCAB_JI_CLOCK_FULL,
  JP_VOCAB_JI_CLOCK_STEM,
  JP_VOCAB_KA_DAY_FULL,
  JP_VOCAB_KA_DAY_STEM,
  JP_VOCAB_NIN_COUNTER_FULL,
  JP_VOCAB_NIN_COUNTER_STEM,
  JP_VOCAB_QUANTITY_FURIGANA_PROMPT_HINT,
  JP_VOCAB_TUTSU_COUNTER_FULL,
  JP_VOCAB_TUTSU_COUNTER_STEM,
  jpVocabExampleHasWrongQuantityFurigana,
  jpVocabExampleHasWrongTutsuCounterFurigana,
} from "@/lib/jp-vocab-quantity-furigana";
 ...
 * 例：入口(いりくち) ≠ いりぐち
 * 亦含数量词错标：六(ろく)つ／九(きゅう)時／二十(にじゅう)歳
 */
export function jpVocabExampleHasWrongJukugoFurigana(text: string): boolean {
  const s = String(text || "");
  if (!s) return false;

  if (jpVocabExampleHasWrongQuantityFurigana(s)) return true;
...
export const JP_VOCAB_JUKUGO_FURIGANA_PROMPT_HINT = `熟语必须整词标假名，禁止按训读拆开；读音须正确（该连浊的必须浊化）：
   ...
   - ${JP_VOCAB_QUANTITY_FURIGANA_PROMPT_HINT.trim()}
