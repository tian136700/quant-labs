...
/**
 * 存库假名标注：汉字（可夹送假名/词尾假名）后半角/全角括号，
 * 如 電車(でんしゃ)、静か(しずか)、焚き火(たきび)。
 * 展示层转成「汉字正下方小字」，勿把括号原文直接塞进 UI。
 *
 * 「焚き火」须整词匹配；但不能把助词「は」等吃进括注（否则
 * 「今日は気分(きぶん)」会误当成一词，漏标检测失效）。
 * 中间假名用 (?!助词) 限制，词尾假名仍可含 か（静か）。
 */
export const JP_VOCAB_PAREN_FURIGANA_RE =
  /([\u4E00-\u9FFF々]+(?:(?![はがをにでとへもやの])[ぁ-んァ-ンヴヵヶー]+[\u4E00-\u9FFF々]+)*[ぁ-んァ-ンヴヵヶー]*)[（(]([ぁ-んァ-ンヴヵヶー]+)[）)]/g;
...
/**
 * 合法假名括注：紧贴词面（可含送假名），括号内仅假名。
 * 须与 JP_VOCAB_PAREN_FURIGANA_RE 同宽，否则「焚き火(たきび)」只会保护「火(…)」，
 * 漏标检测会误报「焚」incomplete；也不能跨过助词「は」把整句吞成一词。
 */
