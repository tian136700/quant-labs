...
export const JP_VOCAB_EXAMPLE_SENTENCES_UPLOAD_SPEC = {
  ...
  rules: [
    ...
    "汉字（可带词尾假名）后立刻半角括号假名：漢字(かな) / 静か(しずか)；括号内只能是假名、不要空格、不要整句读音尾注如 です。(たなかさん…)；禁止句末语法说明括号如 (必要なは必要だ…の形です)；页面展示会转成汉字下方小字",
...
7. 只输出「日语 / 译文：…」交替行；不要行首编号、不要 markdown、不要解释、不要额外语法说明。`;
}
