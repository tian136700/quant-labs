...
${
  input.kind === "grammar"
    ? ""
    : `11. 相关构词（助记，与例句等同一次输出，勿另开请求）：没有自然相关词就不要写（填空/省略即可，禁止硬凑）；只有 1～2 个就写 1～2；多则最多 4～5 行「漢字(かな)：中文」；须含本词汉字；单汉字须同读（允许连浊くち→ぐち、こと→ごと；禁止不同音读，如事=こと勿写食事/大事的「じ」）；多字词先拆部件再举同旁词（会社員→会社(かいしゃ)：公司、店員(てんいん)：店员）；【禁止本词】不要把词条本身再写进相关构词（研修生≠再写研修生）；一词多义用中文逗号「，」（目上：上级，长辈），释义里禁止用「；」；优先 N5～N4（口→入口(いりぐち)：入口）；假名须正确；禁止商务难词。`
}
