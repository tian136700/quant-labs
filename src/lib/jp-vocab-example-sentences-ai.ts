...
${
  input.kind === "grammar"
    ? ""
    : `11. 相关构词（助记，与例句等同一次输出，勿另开请求）：没有自然同读相关词就不要写（填空/省略即可，禁止硬凑）；只有 1～2 个就写 1～2；多则最多 4～5 行「漢字(かな)：中文」；须含本词汉字且读音与本词一致（允许连浊くち→ぐち、こと→ごと；禁止不同音读，如事=こと勿写食事/大事的「じ」）；优先 N5～N4（口→入口(いりぐち)：入口）；假名须正确；禁止商务难词。`
}
