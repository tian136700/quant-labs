...
export type EnLessonProgressStatus = "pending" | "learning" | "completed";

export function getEnLessonProgressStatus(lesson: {
  completed: boolean;
  learning?: boolean;
}): EnLessonProgressStatus {
  if (lesson.completed) return "completed";
  if (lesson.learning) return "learning";
  return "pending";
}

export function enLessonProgressToFields(
...
export function isEnLessonSyncedToVocab(lesson: {
  completed: boolean;
}): boolean {
...
/** 列表排序：学习中 → 未完成 → 已完成 */
export function enLessonProgressSortRank(lesson: {
  completed: boolean;
  learning?: boolean;
}): number {
...
export function enLessonRecentOperationAt(lesson: {
...
export function compareEnLessonsByRecentOperation(
  a: { status_updated_at?: string | null; uploaded_at: string; id: number },
  b: { status_updated_at?: string | null; uploaded_at: string; id: number }
): number {
  const dateCmp = enLessonRecentOperationAt(b).localeCompare(enLessonRecentOperationAt(a));
  if (dateCmp !== 0) return dateCmp;
  return b.id - a.id;
}

export function compareEnLessonsByProgress(
  a: {
    completed: boolean;
    learning?: boolean;
    status_updated_at?: string | null;
  ...
  const rankCmp = enLessonProgressSortRank(a) - enLessonProgressSortRank(b);
  if (rankCmp !== 0) return rankCmp;
  return compareEnLessonsByRecentOperation(a, b);
}

type EnLessonClassScheduleLike = {
...
export function getLessonEarliestClassAt(lesson: EnLessonClassScheduleLike): string | null {
  const primary = getLessonPrimaryClassSchedule(lesson);
  return primary?.class_at ?? null;
}

/** 取课程最早一条预约时段（用于列表排序与合并） */
export function getLessonPrimaryClassSchedule(lesson: EnLessonClassScheduleLike): {
  id: number;
  class_at: string;
  duration_minutes: number | null;
} | null {
...
export function compareEnLessonsByClassTime(
  a: {
    class_schedules?: Array<{
      class_at: string;
      duration_minutes: number | null;
  ...
  if (!aAt && !bAt) return compareEnLessonsByRecentOperation(a, b);
  if (!aAt) return 1;
  if (!bAt) return -1;
...
export function compareEnLessonsByClassTimeDesc(
  a: Parameters<typeof compareEnLessonsByClassTime>[0],
  b: Parameters<typeof compareEnLessonsByClassTime>[1]
): number {
  const aAt = getLessonEarliestClassAt(a);
  const bAt = getLessonEarliestClassAt(b);
  if (!aAt && !bAt) return compareEnLessonsByRecentOperation(a, b);
...
export type EnLessonClassTimeSortOrder = "asc" | "desc";

export function compareEnLessonsByClassTimeOrder(
  order: EnLessonClassTimeSortOrder
): typeof compareEnLessonsByClassTime {
  return order === "asc" ? compareEnLessonsByClassTime : compareEnLessonsByClassTimeDesc;
}
...
export type EnLessonDisplayGroup<T extends { id: number }> = {
  key: string;
...
export function buildEnLessonDisplayGroups<T extends {
  id: number;
  teacher_ids?: number[];
  teacher_other?: string | null;
  class_schedules?: Array<{
  ...
  status_updated_at?: string | null;
  uploaded_at: string;
}>(lessons: T[], sortOrder: EnLessonClassTimeSortOrder = "asc"): EnLessonDisplayGroup<T>[] {
  const sorted = [...lessons].sort(compareEnLessonsByClassTimeOrder(sortOrder));
  const groups: EnLessonDisplayGroup<T>[] = [];
  ...
  for (const lesson of sorted) {
    ...
    const group: EnLessonDisplayGroup<T> = {
...
export function getLessonClassDate(lesson: EnLessonClassScheduleLike): string | null {
...
export function buildLearningClassDayToneMap<T extends EnLessonClassScheduleLike>(
...
export function flattenEnLessonScheduleEvents(
  lessons: Array<Parameters<typeof buildEnLessonScheduleEvents>[0]>
): EnLessonScheduleEvent[] {
  return lessons
    .flatMap((lesson) => buildEnLessonScheduleEvents(lesson))
...
export type EnLessonScheduleEventStatus = "past" | "ongoing" | "upcoming";

export function getEnLessonScheduleEventStatus(
  event: { start: Date; end: Date },
  now = new Date()
): EnLessonScheduleEventStatus {
