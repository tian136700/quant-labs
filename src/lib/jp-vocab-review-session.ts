...
export type JpVocabReviewRoundProgress = {
  /** 本轮分母：计划内今日未抽问的词条数 */
  roundTotal: number;
...
export function computeJpVocabReviewRoundProgress(input: {
  planWordIds: readonly number[];
  currentWordId: number;
  reviewedWordIds: ReadonlySet<number>;
  isQuizzedToday: (wordId: number) => boolean;
}): JpVocabReviewRoundProgress {
...
export function countJpVocabReviewQuizzedInPlan(
  planWordIds: readonly number[],
  isQuizzedToday: (wordId: number) => boolean
): number {
