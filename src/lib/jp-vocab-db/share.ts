...
export async function queryJpVocabSharedToday(
  ...
  const result = await db
    .prepare(
      `SELECT s.id, s.word_id, s.shared_by, s.shared_at, s.share_date,
              ...
              w.related_compounds, w.related_compounds_source,
              w.annotation,
              w.course_label,
              w.oral_frequency,
              w.exam_frequency,
  ...
  const items = (result.results ?? []).map((row) => {
    const word = mapSharedListWordRow({
      ...
      related_compounds: row.related_compounds,
      related_compounds_source: row.related_compounds_source,
 ...
 * 返回新插入条数；无漏发时 0 次写库。
 */
export async function backfillJpVocabCheckedUnsharedShares(
  db: D1Database,
  ...
  if (jpVocabDbState.devStoreEnabled) {
    ...
    if (n > 0) invalidateJpVocabSharedTodayCache();
    return n;
  }
  ...
  const missing = await db
    .prepare(
      excludeWordId == null
        ? `SELECT w.id AS id FROM jp_vocab_word w
           WHERE w.today_check_date = ?1
             AND COALESCE(w.today_check_count, 0) > 0
             AND NOT EXISTS (
               SELECT 1 FROM jp_vocab_shared s
  ...
  const insertSql = `INSERT INTO jp_vocab_shared
    (word_id, shared_by, shared_at, share_date, auto_marked_level)
    VALUES (?1, ?2, ?3, ?4, NULL)`;
  await db.batch(
    ids.map((id) =>
      db.prepare(insertSql).bind(id, sharedBy, ts, today)
    )
  );
  invalidateJpVocabSharedTodayCache();
  return ids.length;
