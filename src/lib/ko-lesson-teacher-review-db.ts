...
import type {
  KoLessonTeacherReviewRecord,
  KoLessonTeacherReviewSortField,
  KoLessonTeacherReviewSummary,
} from "./types";

const SORT_COLUMNS: Record<KoLessonTeacherReviewSortField, string> = {
...
const devRecords: KoLessonTeacherReviewRecord[] = [];
let devNextId = 1;

export function enableKoLessonTeacherReviewDevStore() {
...
function mapRow(row: Record<string, unknown>): KoLessonTeacherReviewRecord {
  return {
    id: Number(row.id),
...
const REVIEW_SELECT = `SELECT id, teacher_id, class_date, score, remark, created_at, updated_at FROM ko_lesson_teacher_review`;

export type SaveKoLessonTeacherReviewInput = {
  id?: number;
  teacher_id: number;
  class_date: string;
  score: number;
...
export type SaveKoLessonTeacherReviewResult =
  | { ok: true; record: KoLessonTeacherReviewRecord }
  | { ok: false; error: string };

export async function saveKoLessonTeacherReview(
  db: D1Database,
  input: SaveKoLessonTeacherReviewInput
): Promise<SaveKoLessonTeacherReviewResult> {
  ...
  if (devStoreEnabled) {
    ...
    if (id > 0) {
      ...
      const updated: KoLessonTeacherReviewRecord = {
        ...devRecords[idx],
    ...
    const created: KoLessonTeacherReviewRecord = {
      id: devNextId++,
  ...
  if (id > 0) {
    const result = await db
      .prepare(
        `UPDATE ko_lesson_teacher_review
    ...
    await db
      .prepare(
        `INSERT INTO ko_lesson_teacher_review (teacher_id, class_date, score, remark, created_at, updated_at)
         VALUES (?1, ?2, ?3, ?4, ?5, ?6)`
      )
  ...
  const record = await getKoLessonTeacherReviewById(db, savedId);
  if (!record) {
    return { ok: false, error: "save_failed" };
  }
...
export async function listKoLessonTeacherReviews(
  db: D1Database,
  teacherId: number,
  sortField: KoLessonTeacherReviewSortField = "updated_at",
  sortOrder: "asc" | "desc" = "desc",
  limit = 500
): Promise<KoLessonTeacherReviewRecord[]> {
  ...
  if (devStoreEnabled) {
    ...
    const sorted = [...filtered].sort((a, b) => {
      const av = a[col as keyof KoLessonTeacherReviewRecord];
      const bv = b[col as keyof KoLessonTeacherReviewRecord];
...
export async function getKoLessonTeacherReviewById(
  db: D1Database,
  recordId: number
): Promise<KoLessonTeacherReviewRecord | null> {
  if (devStoreEnabled) {
...
export async function deleteKoLessonTeacherReviewRecords(
  db: D1Database,
  recordIds: number[]
  ...
  const result = await db
    .prepare(`DELETE FROM ko_lesson_teacher_review WHERE id IN (${placeholders})`)
    .bind(...ids)
    .run();
...
function pickLatestRemarkReview(
  current: KoLessonTeacherReviewRecord | null,
  candidate: KoLessonTeacherReviewRecord
): KoLessonTeacherReviewRecord | null {
...
export async function listKoLessonTeacherReviewSummaries(
  db: D1Database
): Promise<KoLessonTeacherReviewSummary[]> {
  if (devStoreEnabled) {
    const map = new Map<
      number,
      { total: number; count: number; latestRemark: KoLessonTeacherReviewRecord | null }
    >();
  ...
  const { results } = await db
    .prepare(
      `SELECT teacher_id,
              ...
              (
                SELECT remark FROM ko_lesson_teacher_review r2
              ...
              (
                SELECT class_date FROM ko_lesson_teacher_review r2
                WHERE r2.teacher_id = r.teacher_id
                  AND r2.remark IS NOT NULL AND TRIM(r2.remark) != ''
                ORDER BY r2.updated_at DESC, r2.id DESC
                LIMIT 1
              ) AS latest_class_date
       FROM ko_lesson_teacher_review r
       GROUP BY teacher_id`
    )
    .all<{
      teacher_id: number;
