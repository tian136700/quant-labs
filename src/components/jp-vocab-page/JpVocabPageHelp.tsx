    ...
    <div className="jp-vocab-help">
      ...
      {expanded ? (
        <p className="jp-vocab-risk-hint" role="note">
          ...
          ：列表与卡片展示的是「最终抽问得分」= 基础优先级 + 距上次抽问天数 ×{" "}
          {quizTimeWeight}（固定系数，仅作同档打平参考）。
          基础优先级：一般 × 1 + 不熟悉 × 2 − 非常熟悉 × 0.3（保留 1 位小数）。
          今日抽查日序以间隔重复（SRS 到期日）为主；最终得分只在同到期日时打平。
          ...
          单词表默认按当日固定序号（凌晨按 SRS + 最终得分重排）；当天内勾选或刷新页面不会改变顺序（所有老师看到相同顺序）。管理员在「今日抽查数量」中设置目标后，系统为老师生成可见池（当日序号正序 1…N）；今日新入库从未抽查词不进池。跨日自动回到默认设置。管理员可使用「重置 → 今日重置」立即重排并清空当前轮次勾选，统计次数不变。
