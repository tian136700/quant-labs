...
import type { EnLessonKind, EnLessonNote, EnLessonRecord } from "@/lib/types";
...
function persistLessonNotesCache(nextNotes: EnLessonNote[]) {
  patchClientCache<EnLessonApiPayload>(JP_LESSON_CACHE_KEY, (prev) =>
...
function kindLabel(kind: EnLessonKind): string {
  return kind === "grammar" ? "语法" : "单词";
}

function buildItemFields(
  items: string[],
  lessonNotes: EnLessonNote[]
): ItemFields {
...
export function EnLessonNotesPage() {
  const searchParams = useSearchParams();
  const lessonId = Number(searchParams.get("id"));
  const { locale } = useI18n();
  const { user, checking, canAccessEnVocab, openAuthPanel } = useEtrAuth();
  const canEdit = canAccessEnVocab;
  ...
  const [lesson, setLesson] = useState<EnLessonRecord | null>(() => initialCached?.lesson ?? null);
  const [notes, setNotes] = useState<EnLessonNote[]>(() => initialCached?.notes ?? []);
  ...
  const saveNotes = useCallback(
    async (opts?: { auto?: boolean }) => {
      ...
      try {
        ...
        const lessonResult: EnLessonNote[] = [];
        ...
        for (const item of items) {
          ...
          for (const init of initial) {
            if (init.noteId && !currentIds.has(init.noteId)) {
              const res = await fetch("/api/en-lesson/notes", {
                method: "DELETE",
                headers: {
        ...
        if (lesson.completed) {
          const syncRes = await fetch("/api/en-lesson/notes/sync", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              [LOCALE_HEADER]: locale,
