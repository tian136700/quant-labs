...
const STORAGE_KEY_PREFIX = "jp-vocab-teacher-quiz-intro-v1";
...
export function shouldShowJpVocabTeacherQuizIntro(userId: number): boolean {
...
function markJpVocabTeacherQuizIntroDismissed(userId: number): void {
...
function markJpVocabTeacherQuizIntroDismissedForever(userId: number): void {
  if (typeof window === "undefined") return;
  try {
...
export function JpVocabTeacherQuizIntroModal({
  userId,
  open,
  onConfirm,
  onClose,
  ...
  const handleConfirm = () => {
    if (dontShowAgain) {
      markJpVocabTeacherQuizIntroDismissedForever(userId);
    } else {
      markJpVocabTeacherQuizIntroDismissed(userId);
    }
    onConfirm();
  };
  ...
  return createPortal(
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="jp-vocab-teacher-quiz-intro-title"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="jp-vocab-teacher-quiz-intro-header">
          <h2
            id="jp-vocab-teacher-quiz-intro-title"
        ...
        <div className="jp-vocab-teacher-quiz-intro-body">
          <ol className="jp-vocab-teacher-quiz-intro-list">
            <li>
              <strong>抽问学生</strong>：向学生随机提问（可用中文或日文），学生回答后，请根据实际掌握情况勾选「非常熟悉」「一般」或「不熟悉」。卡片上方会显示<strong>答题用时</strong>（正计时），可参考学生思考快慢判断熟练度；勾选后计时停住，下一词重新从 0 开始。
            </li>
            <li>
