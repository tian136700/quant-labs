...
import type { EnLessonTeacher, EnLessonTeacherReviewSummary } from "@/lib/types";
...
export function AdminEnLessonTeachersPage() {
  ...
  const [reviewSummaries, setReviewSummaries] = useState<
    Map<number, EnLessonTeacherReviewSummary>
  >(new Map());
  const [reviewTeacher, setReviewTeacher] = useState<EnLessonTeacher | null>(null);
  ...
  const loadReviewSummaries = useCallback(async () => {
    try {
      const res = await fetch("/api/admin/en-lesson-teacher-review?summary=1", {
        credentials: "include",
      });
      const data = (await res.json()) as {
        ok?: boolean;
        summaries?: EnLessonTeacherReviewSummary[];
      };
      if (!data.ok) return;
      const map = new Map<number, EnLessonTeacherReviewSummary>();
  ...
  const loadTeachers = useCallback(async () => {
    ...
    try {
      const [teachersRes, summariesRes] = await Promise.all([
        fetch("/api/admin/en-lesson-teachers", { credentials: "include" }),
        fetch("/api/admin/en-lesson-teacher-review?summary=1", {
          credentials: "include",
        }),
      ]);
  ...
  const startEdit = (teacher: EnLessonTeacher) => {
    ...
    <div className="admin-page">
      <div className="page-hero">
        ...
        <p className="hint">
          ...
          {" · "}
          <a href={enLessonPath()}>{locale === "zh" ? "英语新课" : "JP lessons"}</a>
          {" · "}
