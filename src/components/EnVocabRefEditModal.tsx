...
type Props = {
  ...
  canEdit: boolean;
  onClose: () => void;
  onUpdated: (ref: EnVocabRef, lesson: EnLessonRecord) => void;
  onNeedAuth: () => void;
};
...
export function EnVocabRefEditModal({
  open,
  lessonId,
  refKey,
  refMeta,
  ...
  const currentMediaUrl = refKey
    ? enVocabRefApiPath(refKey, { v: refMeta?.updated_at })
    : "";
  const currentViewerUrl = refKey
    ? enVocabRefViewerPath(refKey, refMeta?.updated_at)
    : "";
