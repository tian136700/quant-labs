  ...
  return (
    <div className="ko-pron-select-page">
      ...
      <p className="ko-pron-select-hint">
        先勾选多条（可全选当前筛选结果），再选「批量加入抽问」或「批量加入复习」。两池独立，同一字母可两边都进。抽问：立刻进入「韩语发音抽问」管理员端，同日勾选次日才进老师今日抽查池。复习：进入「韩语发音复习」自测读音。入库后不可取消。「导出随机抽问卡片」只导出已入抽问的字母（乱序编号、不含罗马音），无需再勾选一遍，方便发给不敢点链接的老师线下抽问。
      </p>
        ...
        <>
          ...
          <div className="ko-pron-select-batch-bar">
            ...
            <button
              ...
              title={
                exportCount > 0
                  ? `导出已入抽问的 ${exportCount} 个字母（乱序编号，仅韩语字母）`
                  : "还没有已入抽问的字母，请先批量加入抽问"
              }
            >
              {exportBusy
