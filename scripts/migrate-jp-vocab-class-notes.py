#!/usr/bin/env python3
"""jp_vocab_word 增加 class_notes 列（可重复执行）。"""
...
def main() -> int:
    ...
    remote = args.remote
    if column_exists(remote, "jp_vocab_word", "class_notes"):
        print("[migrate-jp-vocab-class-notes] already exists, skip", flush=True)
        return 0

    run_wrangler(
        remote,
        "ALTER TABLE jp_vocab_word ADD COLUMN class_notes TEXT;",
    )
    print("[migrate-jp-vocab-class-notes] done", flush=True)
    return 0
