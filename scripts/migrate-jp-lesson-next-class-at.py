#!/usr/bin/env python3
"""jp_lesson 增加 next_class_at 列（可重复执行）。"""
...
def main() -> int:
    ...
    remote = args.remote
    if column_exists(remote, "jp_lesson", "next_class_at"):
        print("[migrate-jp-lesson-next-class-at] already exists, skip", flush=True)
        return 0

    run_wrangler(remote, "ALTER TABLE jp_lesson ADD COLUMN next_class_at TEXT;")
    print("[migrate-jp-lesson-next-class-at] done", flush=True)
    return 0
