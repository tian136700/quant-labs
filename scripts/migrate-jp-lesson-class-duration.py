#!/usr/bin/env python3
"""jp_lesson 增加 class_duration_minutes 列（可重复执行）。"""
...
def main() -> int:
    ...
    remote = args.remote
    if column_exists(remote, "jp_lesson", "class_duration_minutes"):
        print("[migrate-jp-lesson-class-duration] already exists, skip", flush=True)
        return 0
    ...
    print("[migrate-jp-lesson-class-duration] done", flush=True)
    return 0
