#!/bin/bash
# 临时：单用法语法例句补到 3 条（每分钟 1 条；队列空自动卸 launchd）
#
#   bash scripts/jp-vocab-fill-single-usage-examples-online-stage.sh
#   FORCE=1 bash scripts/jp-vocab-fill-single-usage-examples-online-stage.sh
#   MAX_ROUNDS=2 bash scripts/jp-vocab-fill-single-usage-examples-online-stage.sh
set -euo pipefail
...
LOCK_DIR="${CONFIG_DIR}/jp-vocab-fill-single-usage-examples-online.lock.d"
STATE_FILE="${CONFIG_DIR}/jp-vocab-fill-single-usage-examples-online.last_success"
DONE_PATH="${CONFIG_DIR}/jp-vocab-fill-single-usage-examples-online-DONE.switch"
OWNER="jp-vocab-fill-single-usage-examples-online"
...
dirlock_acquire "$LOCK_DIR" "$OWNER" \
  "${JP_VOCAB_FILL_SINGLE_USAGE_EXAMPLES_LOCK_STALE_SECONDS:-3600}"

cd "$ROOT"
echo "$(date '+%F %T') ${OWNER}: start max_rounds=${MAX_ROUNDS}"

set +e
python3 "$ROOT/scripts/jp-vocab-fill-single-usage-examples-online-api.py" \
