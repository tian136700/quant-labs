#!/bin/bash
# 临时：线上英语「口语/考试频率」补全（每分钟 1 条；队列空自动卸 launchd）
#
#   bash scripts/en-vocab-fill-frequency-online-stage.sh
#   FORCE=1 bash scripts/en-vocab-fill-frequency-online-stage.sh
#   MAX_ROUNDS=2 bash scripts/en-vocab-fill-frequency-online-stage.sh
set -euo pipefail
...
LOCK_DIR="${CONFIG_DIR}/en-vocab-fill-frequency-online.lock.d"
STATE_FILE="${CONFIG_DIR}/en-vocab-fill-frequency-online.last_success"
DONE_PATH="${CONFIG_DIR}/en-vocab-fill-frequency-online-DONE.switch"
OWNER="en-vocab-fill-frequency-online"
LABEL="com.infoquests.en-vocab-fill-frequency-online"
...
dirlock_acquire "$LOCK_DIR" "$OWNER" \
  "${EN_VOCAB_FILL_FREQUENCY_LOCK_STALE_SECONDS:-3600}"

cd "$ROOT"
echo "$(date '+%F %T') ${OWNER}: start max_rounds=${MAX_ROUNDS}"

set +e
python3 "$ROOT/scripts/en-vocab-fill-frequency-online-api.py" --max-rounds "$MAX_ROUNDS"
status=$?
set -e
