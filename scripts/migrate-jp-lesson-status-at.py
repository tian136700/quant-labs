#!/usr/bin/env python3
"""jp_lesson 增加 status_updated_at 列（可重复执行）。"""
...
def main() -> int:
    ...
    remote = args.remote
    if column_exists(remote, "jp_lesson", "status_updated_at"):
        print("[migrate-jp-lesson-status-at] already exists, skip", flush=True)
        return 0

    run_wrangler(
        remote,
        "ALTER TABLE jp_lesson ADD COLUMN status_updated_at TEXT;",
    )
    # 已有记录：若 updated_at 与 uploaded_at 不同，说明曾切换过完成状态
    run_wrangler(
        remote,
        """
        UPDATE jp_lesson
        ...
        WHERE status_updated_at IS NULL
          ...
          AND updated_at != uploaded_at;
        """.strip(),
    )
    print("[migrate-jp-lesson-status-at] done", flush=True)
    return 0
