...
- usage: 编号中文用法；每条必须带出现频次 [1]～[10]（10=该词最常见用法），形如「1. [8] 介词：…」；组数=真实不同核心义项数（1 种就 1 条，禁止硬凑 2 条）；禁止按对象/场景硬拆同一义（如 attractive 的「对客户有吸引力」与「外表好看」须合并为 1 条）；只有词性/词典义/固定结构真不同才拆条；每条只标一种词性，禁止「动词/名词」等含糊写法（例句是名词就标名词；名词与动词义都常用则拆成两条）；选题按上方分类语境高频，正文禁止考试品牌名。也可返回数组 [{{"text":"…","frequency":8}},…]（frequency 必填 1～10）
