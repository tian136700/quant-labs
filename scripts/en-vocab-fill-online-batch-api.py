...
SYSTEM = (
    ...
    "or \"account；账户\". Ordinary non-abbreviation words: Chinese senses only "
    "(do not invent an English expansion). "
    ...
    "(e.g. fail: do NOT split 'system/project fails' vs 'fail an exam/check'—merge as one verb sense; "
    "freeze: do NOT split 'freeze salaries' vs 'freeze an account'—merge as one verb sense; "
    "keep a separate noun line only if the noun sense is also common); "
...
def build_prompt(row: dict[str, Any], needs: dict[str, bool]) -> str:
    ...
    if not category or category == "雅思托福":
        ...
        category_focus = (
            "选题按美国 F-1 学生签证面签的高频语境，"
            "优先学习目的、学校与专业、资金来源、CPT、毕业后回国计划、"
            "移民倾向相关问答；释义与例句也要偏面签口语。"
        )
    elif "IT面试" in category or "it interview" in category.lower():
        category_focus = (
...
def generate_bundle(row: dict[str, Any], needs: dict[str, bool]) -> dict[str, Any]:
    ...
    if needs.get("pos"):
        ...
        if pos:
            out["pos"] = rewrite_pos_for_lemma(str(row.get("word") or ""), pos)
    ...
    if needs.get("usage"):
        ...
        if not usage:
            ...
            if usage and not out.get("pos") and needs.get("pos"):
                ...
                if pos:
                    out["pos"] = rewrite_pos_for_lemma(
                        str(row.get("word") or ""), pos
                    )
