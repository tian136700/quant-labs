#!/usr/bin/env python3
"""下课（开课+课时）后再过 10 分钟：自动禁用老师关联账号。

与日语教案上传共用 JP_REVIEW_UPLOAD_TOKEN；
由 teacher-user-post-class-disable.sh / launchd（每 10 分钟）调度。
"""
...
DEFAULT_API_URL = (
    "https://finance.info-quests.com/api/admin/teacher-user-post-class-disable"
)
HTTP_USER_AGENT = (
...
def print_result(payload: dict) -> None:
    ...
    teachers = payload.get("teachers_due") or []
    print(
        f"[teacher-user-post-class-disable-api] grace_ms={payload.get('grace_ms')} "
        f"teachers_due={len(teachers)} {teachers}",
        flush=True,
    )
    disabled = payload.get("disabled") or []
    print(f"  disabled={len(disabled)}", flush=True)
    ...
    if skipped:
        ...
        for item in skipped:
            print(
                f"    skip user_id={item.get('user_id')} "
                f"username={item.get('username')} reason={item.get('reason')}",
                flush=True,
            )
    print(
        f"[teacher-user-post-class-disable-api] done, "
...
def main() -> int:
    ...
    cfg = load_env_file("teacher-user-post-class-disable.env")
    parser = argparse.ArgumentParser(
        description="Enable disabled teacher accounts within 2h before class."
    )
    parser.add_argument(
        "--api-url",
        default=cfg.get("TEACHER_USER_POST_CLASS_DISABLE_URL", DEFAULT_API_URL),
    )
