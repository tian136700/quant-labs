#!/usr/bin/env python3
"""jp_lesson 增加 completed 列（可重复执行）。"""
...
def main() -> int:
    ...
    remote = args.remote
    if column_exists(remote, "jp_lesson", "completed"):
        print("[migrate-jp-lesson-completed] already exists, skip", flush=True)
        return 0

    run_wrangler(
        remote,
        "ALTER TABLE jp_lesson ADD COLUMN completed INTEGER NOT NULL DEFAULT 0;",
    )
    print("[migrate-jp-lesson-completed] done", flush=True)
    return 0
