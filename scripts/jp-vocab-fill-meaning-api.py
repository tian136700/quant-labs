...
SYSTEM = (
    ...
    "词性：中文（名词/他动词/自动词/动词/い形容词…），多词性用 /。"
    "动词尽量区分：他动词→「他动词」；自动词→「自动词」；不分或不清楚→「动词」。"
    "例句：只要比较常用的用法；日语行须汉字后半角括号假名；下一行「译文：」中文。"
    "熟语整词标假名；该连浊必须浊化：✅出発(しゅっぱつ)／入口(いりぐち)／友達(ともだち)；❌出(で)発(ぱつ)、❌入口(いりくち)、❌友達(ゆうだち)（夕立）、❌日本(にっぽん)語(ご)。"
    "「〜つ」个数：✅一(ひと)つ／六(むっ)つ；❌六(ろく)つ（ろく＝六点，不是六个）。"
    "数量词训读：✅一人(ひとり)／二日(ふつか)／九(く)時／二十歳(はたち)；❌九(きゅう)時／二十(にじゅう)歳(さい)。"
    "不要 markdown、不要解释过程。"
)
...
POS_TOKEN_RE = re.compile(
    r"^(名词|他动词|自动词|动词|い形容词|な形容词|形容词|副词|助词|接続词|接续词|"
...
def call_api(
    *,
    ...
    """POST Worker；≥5s 间隔 + 429/1027 处理见 worker_fill_http。"""
...
def run_one_fill(
    *,
    ...
    if not prompt:
        ...
        if need_pos:
            jobs.append(
                "词性：一行中文；动词尽量写「他动词」或「自动词」，不分则「动词」；多词性用 /"
            )
        if need_examples:
