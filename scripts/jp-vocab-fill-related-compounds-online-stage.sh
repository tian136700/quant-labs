...
LOCK_DIR="${CONFIG_DIR}/jp-vocab-fill-related-compounds-online.lock.d"
STATE_FILE="${CONFIG_DIR}/jp-vocab-fill-related-compounds-online.last_success"
DONE_PATH="${CONFIG_DIR}/jp-vocab-fill-related-compounds-online-DONE.switch"
OWNER="jp-vocab-fill-related-compounds-online"
...
dirlock_acquire "$LOCK_DIR" "$OWNER" \
  "${JP_VOCAB_FILL_RC_ONLINE_LOCK_STALE_SECONDS:-3600}"

cd "$ROOT"
echo "$(date '+%F %T') ${OWNER}: start"

set +e
python3 "$ROOT/scripts/jp-vocab-fill-related-compounds-online-api.py"
status=$?
set -e
