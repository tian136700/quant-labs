#!/usr/bin/env python3
"""jp_lesson 增加 teacher_other 列（可重复执行）。"""
...
def main() -> int:
    ...
    remote = args.remote
    if column_exists(remote, "jp_lesson", "teacher_other"):
        print("[migrate-jp-lesson-teacher-other] already exists, skip", flush=True)
        return 0

    run_wrangler(remote, "ALTER TABLE jp_lesson ADD COLUMN teacher_other TEXT;")
    print("[migrate-jp-lesson-teacher-other] done", flush=True)
    return 0
