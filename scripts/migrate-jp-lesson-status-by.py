#!/usr/bin/env python3
"""jp_lesson 增加 status_updated_by 列（可重复执行）。"""
...
def main() -> int:
    ...
    remote = args.remote
    if column_exists(remote, "jp_lesson", "status_updated_by"):
        print("[migrate-jp-lesson-status-by] already exists, skip", flush=True)
        return 0

    run_wrangler(
        remote,
        "ALTER TABLE jp_lesson ADD COLUMN status_updated_by TEXT;",
    )
    print("[migrate-jp-lesson-status-by] done", flush=True)
    return 0
