...
普通老师 completed_at + 1h；带读账号（欣欣等）+ 2h。
与日语教案上传共用 JP_REVIEW_UPLOAD_TOKEN；由 teacher-user-quiz-complete-disable.sh / launchd 调度。
"""
...
DEFAULT_API_URL = (
    "https://finance.info-quests.com/api/admin/teacher-user-quiz-complete-disable"
)
HTTP_USER_AGENT = (
...
def print_result(payload: dict) -> None:
    ...
    print(
        f"[teacher-user-quiz-complete-disable-api] date={payload.get('date')} "
    ...
    for item in disabled:
        print(
            ...
            f"quiz_date={item.get('quiz_date')} "
            f"completed_at={item.get('completed_at')} "
            f"disable_after_at={item.get('disable_after_at')}",
            flush=True,
        )
    ...
    print(
        f"[teacher-user-quiz-complete-disable-api] done, "
...
def main() -> int:
    ...
    cfg = load_env_file("teacher-user-quiz-complete-disable.env")
    parser = argparse.ArgumentParser(
        description="Disable teacher accounts after daily jp-vocab quiz completion delay."
    )
    parser.add_argument(
        "--api-url",
        default=cfg.get("TEACHER_USER_QUIZ_COMPLETE_DISABLE_URL", DEFAULT_API_URL),
    )
