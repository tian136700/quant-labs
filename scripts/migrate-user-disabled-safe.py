...
def main() -> int:
    ...
    print(f"[migrate-user-disabled] target={label}", flush=True)

    if column_exists(remote, "etr_users", "disabled"):
        print("[migrate-user-disabled] disabled column already exists, skip", flush=True)
        return 0
    ...
    print("[migrate-user-disabled] done", flush=True)
    return 0
