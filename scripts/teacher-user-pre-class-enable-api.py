#!/usr/bin/env python3
"""开课前 2 小时内：禁用态老师关联账号自动启用。

与日语教案上传共用 JP_REVIEW_UPLOAD_TOKEN；
由 teacher-user-pre-class-enable.sh / launchd（每 10 分钟）调度。
"""
...
DEFAULT_API_URL = (
    "https://finance.info-quests.com/api/admin/teacher-user-pre-class-enable"
)
HTTP_USER_AGENT = (
...
def print_result(payload: dict) -> None:
    ...
    teachers = payload.get("teachers_with_upcoming_class") or []
    print(
        f"[teacher-user-pre-class-enable-api] within_ms={payload.get('within_ms')} "
        f"teachers={len(teachers)} {teachers}",
        flush=True,
    )
    ...
    print(
        f"[teacher-user-pre-class-enable-api] done, "
...
def main() -> int:
    ...
    cfg = load_env_file("teacher-user-pre-class-enable.env")
    parser = argparse.ArgumentParser(
        description="Enable disabled teacher accounts within 2h before class."
    )
    parser.add_argument(
        "--api-url",
        default=cfg.get("TEACHER_USER_PRE_CLASS_ENABLE_URL", DEFAULT_API_URL),
    )
