...
WORD_SYSTEM = (
    ...
    "【相关构词·同一次输出】related_compounds：含本词汉字、且读音与本词一致的简单词（口→入口）；"
    "允许连浊（くち→ぐち、こと→ごと）；禁止不同音读（事=こと 勿写 食事/大事 的じ）；"
    "【禁止本词】不要把词条本身写进相关构词（研修生≠再写研修生；企業≠再写企業）；相关=别的词；"
    "【整词假名·必守】每行「漢字(かな)：中文」——假名括号包住整词："
...
GRAMMAR_SYSTEM = (
    ...
    "动词辞书形须写成「动词辞书形（动词原形）」；"
    "❌禁止五段／一段／カ变／サ变／サ変動詞等学术用语（学生听不懂）；"
    "❌禁止无「＋」的长散文（会被剥空导致 connection_invalid:empty）；"
    ...
    "ない形须按词尾「去掉…加…＋ない｜如「…→…」」，"
    "❌禁止「一類動詞（五段動詞）」「う段→あ段」散文，❌禁止接序下「例：」行）；"
