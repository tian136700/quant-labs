...
WORD_SYSTEM = (
    ...
    "❌この셔츠を着ます（셔츠＝韩语）→ ✅このシャツを着ます；"
    "❌駅の近く에 スーパーがあります（에＝韩语助词）→ ✅駅の近くに スーパーがあります。"
    "N5～N4 短句，焦点在本单词，不要塞难语法。"
    "条数：释义含 / 时每段 1 句；否则 max(2, 常用用法数)。"
    "な形容词「〜だ」造句用词干，不必写「だ」。"
    "【一类形容词过去·必守】かった后接です／ですね，禁止叠でした："
    "❌面白かったでした → ✅面白かったです；名词／な形容词过去才用でした。"
    "【間／あいだ·必守】须造 AとBの間／～の間；禁止「忙しい間です」＋「我现在很忙」；"
    "「の間に」=之内／期间，禁止译成「……后」。"
    "【静かに話す·必守】译文写「请小声说话／请轻声说」，禁止「请安静地说话」（中文听着像矛盾）。"
    "【注意する·必守】小心某事接「に」：❌約束を注意 → ✅車に注意／約束に注意。"
    "【好き／嫌い·必守】对象接「が」：❌魚は嫌いです → ✅魚が嫌いです（は表对比，初学卡片勿用）。"
    "【相談する·必守】人に＝向/找某人商量；人と＝和某人一起商量；译文勿与助词对调。"
    "【例句用词】须自然用到该词条：优先写词条汉字（如「貰う」写成 貰(もら)う），"
    "每个汉字立刻半角括号假名。"
    "【每句整词·必守】每一句日语都必须出现词条整词或 reading 假名；"
    "禁止只写词条里一个字或近义词：❌葉子/はっぱ→葉(は)；❌事故第二句只写注意、译文才写意外。"
    "例外：词条是罕用汉字、日常多用假名动词时（如 戴＝かぶる／つける），"
    "例句可写读音活用（かぶって／つける），不必硬塞词条汉字；"
    "词条本身全假名（如 バーゲンかいじょう）时，例句写汉字+括注（バーゲン会場(かいじょう)）也算用到；"
    "接头辞词条（お～／ご～）时，例句写 お名前／お水／ご飯 等敬语前缀即可；"
    "～する サ变（スケッチする等）可用ます形／た形（スケッチします／しました），算用到该词；"
    "禁止整句既无词条汉字也无 reading 假名。"
    "词条本身无汉字时，例句须出现词条假名串（含活用词干）。"
...
GRAMMAR_SYSTEM = (
    ...
    "【禁止韩文·必守】例句日语行禁止 Hangul（한글）；外来语／助词用日语片假名与假名，"
    "❌シャツ写成셔츠、❌に写成에。"
    ...
    "【语法核·必守】例句须出现语法假名核；可用辞书形或ます形"
    "（ようにする／ようにします；ことができる／ことができます 均可）。"
    "用法「」内点名的形态须能在对应例句里对上（辞书形↔ます形可互通）。"
    "【接序说明·必守】多段「｜说明」须按形态区分，禁止三段抄同一句"
    "（→ repeated_identical_notes）；标本 id=521：辞书形／ている形／た形说明各不同。"
    "组数=真实常用用法数；禁止多造例句；例句接续须对应该条用法（た形／原形／て形勿张冠李戴）。"
    "例句只用简单词；须场景自洽、有头有尾（前提与后半结果/建议自然衔接）；"
    "禁止无厘头硬凑（❌来るなら、どうぞ入ってください／来的话请进；✅雨なら、傘を持っていってください）。"
    "句中每个汉字须半角括号假名；译文行禁止写成无标签的中文句（否则会被当成日语漏标）。"
    ...
    "ない形：一类「词尾う段变あ段＋ない｜如…」一行 + 特殊「ある」→「ない」另起 + 二类／三类；"
    "❌不要把一类く／む／ぬ…每个词尾各占一行；"
    "❌无「＋｜」散文「一類動詞（五段動詞）」→ ✅教材「一类动词」表行；"
    "❌接序下「例：」行 → ✅例子写在「｜」后说明列）；"
