...
PAIR_SYSTEM = (
    ...
    "❌ 列举/并列（～し／～たり等）禁止写「由多方面因素共同导致」「共同导致」等过强因果；"
    "可写「列举理由/情况」「又…又…」；后文「常接感想/结论」可以，勿写成语法本身=导致。"
    ...
    "非变形词条：每条用法编号后须带 [口语n|考试m]（口语/考试出现分各 1～10）。"
    "【单用法 → 3 条例句】只有 1 种用法时：用法仍只写 1 条；例句须恰好 3 句，"
    "分别覆盖【接序】里不同词类/形态（如一类形容词／二类形容词／名词）；"
    "接续不足 3 种时仍造 3 句、换场景，禁止三句同一接续。"
    "【多用法 → 1:1】有 2 种及以上用法时：每条用法下恰好 1 条例句+「译文：」；"
    "禁止给某一用法多造几句（否则卡片会错挂）。"
    ...
    "例句只用简单词、不叠更难语法。例句须场景自洽、有头有尾：前提与后半结果/建议自然衔接；"
    "禁止无厘头硬凑（❌来るなら、どうぞ入ってください／来的话请进；✅雨なら、傘を持っていってください）。"
    "不要 markdown；不要写「JLPT」「能力考」字样。"
    "全部写完后另起一行「【接序】」。接序对学生友好：能写「原形＋本语法」就写「原形」，"
    "少用「普通形」「现在肯定为词干」；な／名词特殊时短句说明（不加だ／加だ）。"
    "所有定时补全输出的接序统一参照 id=521「～かもしれない」模板：每段「词类／形态＋接什么｜短说明」。"
...
CONJ_PAIR_SYSTEM = (
    ...
    "所有定时补全输出的接序都按该 id=521 模板，不得改成散文或其它私有格式。"
    ...
    "ない形示例（一类通用一行 + 特殊 + 二／三类；❌勿按く／む…逐词尾行）："
    "一类动词词尾う段变あ段＋ない｜如「書く→書かない」「飲む→飲まない」（「う」尾变「わ」如「買う→買わない」）；"
    "一类动词「ある」换成「ない」＋ない｜存在动词特殊；二类动词去掉「る」加「ない」＋ない｜如「食べる→食べない」；"
    "三类动词「する」换成「しない」＋しない｜如「勉強する→勉強しない」；三类动词「くる」换成「こない」＋こない｜如「来る→来ない」。"
    "动词分类对照（❌禁止左边 → ✅必须写右边）：五段／五段动词／一類動詞 → 一类动词；一段／一段动词 → 二类动词；カ变／サ变／する・くる不规则 → 三类动词；❌禁止「一类动词（五段）」括注。"
    "变形写法对照：❌无「＋｜」散文「将词尾变为…」→ ✅表行；❌ない形一类按く／む／ぬ…每个词尾各占一行 → ✅一类「词尾う段变あ段＋ない｜如…」一行，特殊「ある」另起，再二类／三类；て形因音便可按词尾分行；❌接序下另写「例：」行 → ✅例子写在「｜」后说明列；说明内勿用「／」。"
)

CONTRAST_PAIR_SYSTEM = (
    "这是日语读音/形态「对比区别」课（如 何＝なに／なん、くれる／もらう），不是多义句型用法清单。"
    ...
    "【接序】只写形态公式（须含「＋」）；❌接序禁止夹用法说明（主语是谁／受益者是谁／恩惠流向／必须是第三方／可互换）；"
    "「｜」后只允许接续短注（如「给东西」）；多形态用「；」每段自带「＋」，勿用「が／は」斜杠串助词。"
    "授受对比接序样例：用法1: 他人＋が＋我＋に＋名词＋をくれる｜给东西；动词て形＋くれる｜帮忙做事"
...
CONNECTION_ONLY_SYSTEM = (
    ...
    "第一行必须是「【接序】」，下面 2～6 行。"
    "所有定时补全输出接序统一按 id=521 模板：每段「词类／形态＋接什么｜短说明」。"
