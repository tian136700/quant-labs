#!/usr/bin/env python3
"""jp_lesson_manual_schedule 表：日程页手动添加的独立日程（可重复执行）。"""
...
def main() -> int:
    ...
    if not table_exists(remote, "jp_lesson_manual_schedule"):
        run_wrangler(
            remote,
            """
            CREATE TABLE jp_lesson_manual_schedule (
        ...
        print(
            "[migrate-jp-lesson-manual-schedule] created jp_lesson_manual_schedule",
            flush=True,
        )
    else:
        print(
            "[migrate-jp-lesson-manual-schedule] jp_lesson_manual_schedule exists, skip create",
            flush=True,
        )
    ...
    print("[migrate-jp-lesson-manual-schedule] done", flush=True)
    return 0
