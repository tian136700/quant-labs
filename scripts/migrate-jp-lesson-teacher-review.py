#!/usr/bin/env python3
"""日语新课上课老师评价 jp_lesson_teacher_review 表：仅增量创建，不删数据。"""
...
def main() -> int:
    ...
    print(f"[migrate-jp-lesson-teacher-review] target={label}", flush=True)

    if table_exists(remote, "jp_lesson_teacher_review"):
        print(
            "[migrate-jp-lesson-teacher-review] jp_lesson_teacher_review already exists, skip",
            flush=True,
        )
        return 0
    ...
    run_wrangler(
        remote,
        """
        CREATE TABLE jp_lesson_teacher_review (
    ...
    print("[migrate-jp-lesson-teacher-review] created jp_lesson_teacher_review", flush=True)
    ...
    print("[migrate-jp-lesson-teacher-review] done", flush=True)
    return 0
