...
def main() -> int:
    ...
    print(f"[migrate-login-link] target={label}", flush=True)

    if table_exists(remote, "etr_login_links"):
        print("[migrate-login-link] etr_login_links already exists, skip", flush=True)
        return 0
    ...
    print("[migrate-login-link] done", flush=True)
    return 0
