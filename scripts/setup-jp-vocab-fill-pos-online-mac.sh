#!/bin/bash
# 安装「临时」日语线上词性补全：每分钟 1 条，队列空自动卸掉。
#
#   bash scripts/setup-jp-vocab-fill-pos-online-mac.sh
#   # 强制重跑（清 DONE 标记后再装）：
#   FORCE=1 bash scripts/setup-jp-vocab-fill-pos-online-mac.sh
set -euo pipefail
...
EXAMPLE="$ROOT/scripts/com.infoquests.jp-vocab-fill-pos-online.plist.example"
INTERVAL="${JP_VOCAB_FILL_POS_ONLINE_INTERVAL_SECONDS:-180}"

mkdir -p "$CONFIG_DIR" "$LOG_DIR" "${HOME}/Library/LaunchAgents"

chmod +x "$ROOT/scripts/jp-vocab-fill-pos-online-stage.sh"
chmod +x "$ROOT/scripts/jp-vocab-fill-pos-online-api.py"
...
echo "  手工试跑: bash $ROOT/scripts/jp-vocab-fill-pos-online-stage.sh"
