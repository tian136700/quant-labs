...
| **老师端正序/随机抽查卡片**（须勾选熟悉程度才能下一词；未完成前列表不可改选；点「完成」弹出祝贺） | `JpVocabPage.tsx` → `finishTeacherQuiz`、`teacherQuizLocksTable`；`JpVocabTeacherQuizFlashcardModal.tsx`；`JpVocabDailyQuizCompleteModal.tsx` |
