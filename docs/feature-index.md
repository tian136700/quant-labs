...
| **释义补全**（`list_missing` / `apply` / **`clear_all`**；多义用「；」最多 3、常用在前；**无本机 Ollama 定时**；**tokken Anthropic** 与英语线上同套，硬限流 ≥60s/条；新课**不同步**释义） | `POST /api/jp-vocab/fill-meaning`；`jp-vocab-fill-meaning.ts` / `jp-vocab-meaning-ai.ts`；`meaning_source`；`scripts/jp-vocab-fill-meaning-api.py` + `scripts/lib/paid_anthropic_client.py`；规则 `.cursor/rules/jp-vocab-fill-meaning.mdc` |
