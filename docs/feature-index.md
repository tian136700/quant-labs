...
| **随手画**（图片 / **PDF 按页**批注：画笔 / **涂抹** / 直线 / 文字 / 缩放；PDF 翻页保留各页笔画，保存时再合成 PDF 覆盖；打开默认放大约「点两次」；可下载或「保存为最新教案」；**仅保存成功后**「查看」页按 `updated_at` 静默换图） | `LessonAnnotateModal` + `lesson-annotate-pdf.ts` / `useLessonAnnotatePdfPages`；**imageUrl 必须用** `jpVocabRefApiPath` / `enVocabRefApiPath`（勿传查看页 HTML）；PDF 传 `mediaType: "pdf"`；保存 `POST /api/jp-lesson/ref/replace`；规则 `.cursor/rules/lesson-annotate-image-url.mdc`、`.cursor/rules/vocab-ref-view-live-refresh.mdc`；回归 `scripts/check_lesson_annotate_image_url.py` |
