...
| **随手画**（图片教案批注：画笔 / **涂抹**（拖拽框选正方/长方形后**涂黑**盖错处）/ 直线 / 文字 / 缩放；可下载或「保存为最新教案」覆盖本条） | `JpLessonAnnotateModal.tsx`（英语 `EnLessonAnnotateModal.tsx`）；列表点「随手画」；**imageUrl 必须用** `jpVocabRefApiPath` / `enVocabRefApiPath`（勿传 `refViewUrl` 查看页 HTML）；保存 `POST /api/jp-lesson/ref/replace`；规则 `.cursor/rules/lesson-annotate-image-url.mdc`；回归 `scripts/check_lesson_annotate_image_url.py` |
