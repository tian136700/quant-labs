...
| `/en-vocab/admin` | 英语抽背-管理员端 | `src/app/en-vocab/admin/page.tsx` | `EnVocabPage variant="admin"` | 全库、导出 Excel、批量删除、重置（**今日/全部重置须同时清 `en_vocab_shared`**，否则「已共享」仍锁熟悉程度）；列表可直接改熟悉程度（不强制进抽查卡片） |
