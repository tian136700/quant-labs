...
| `/jp-vocab/admin` | **日语抽问-管理员端**（全库、设今日抽查数量、久未复习抬升权重、导出、预览卡片） | `src/app/jp-vocab/admin/page.tsx` | `JpVocabPage variant="admin"` | 同上（设目标等管理操作用 admin API） | 与老师端**共用同一张表 / 同一套 API**；UX 由 `variant` 区分 | `admin`；非管理员进此 URL 会 redirect 到 `/jp-vocab` |
...
| **久未复习抬升权重**（`final_score = priority + days × weight`；**从未抽查不算分、日序默认置顶**；默认权重 0.1；管理员可改；次日凌晨或今日重置重排） | `JpVocabQuizTimeWeightAdmin.tsx`；`jpVocabAppliesFinalQuizScore` / `jpVocabFinalQuizScoreOrNull`；`sortJpVocabWordsForDailyOrder`；规则 `.cursor/rules/jp-vocab-quiz-time-weight.mdc`；回归 `scripts/check_jp_vocab_quiz_score.py` |
...
| 学生点「查看老师正在抽查的单词」、老师卡片提示已自行查看；**获取不到 / 很慢**（老师 live 未同步成功） | `JpVocabStudyPage.tsx` → `peekTeacherQuizWord`；`POST /api/jp-vocab/teacher-quiz-live`；`useJpVocabTeacherQuiz.ts` → `syncTeacherQuizLiveWord`（须 `res.ok` + 失败重试）；`peekJpVocabTeacherQuizLiveWord`（`bypassCache`）；规则 `.cursor/rules/vocab-teacher-quiz-live-sync.mdc`；回归 `scripts/check_vocab_teacher_quiz_live_sync.py` |
...
| **抽问/带读/学生/复习卡片**（**同 UI**：老师抽问、课堂带读、学生收到自动弹卡、学生 peek；一律 `JpVocabTeacherQuizFlashcardModal`；带读 `mode="coach"` 熟悉程度**展示不可勾选**（勿隐藏）；列表操作「查看该带读卡片」=`previewMode` 与点「带读」同卡；学生 `mode="study"`；有例句都显示；**例句标题旁「复制全部」**一键复制全部例句（含括号假名 + 译文）；老师已发送则 peek 按钮变灰勿再弹；备注合并勿冲例句；**复习卡** `JpVocabAdminReviewFlashcardModal` 同样式，无熟悉程度，未展开只露汉字） | `JpVocabStudyPage.tsx` → `mode="study"`；`JpVocabCoachPage.tsx` → `mode="coach"` + `openCoachCardPreview`；`JpVocabTeacherQuizFlashcardModal.tsx`；`JpVocabAdminReviewFlashcardModal.tsx`；`JpVocabExampleSentenceCopyButton.tsx`；`jpVocabExampleSentencesCopyText`；`JpVocabTeacherQuizFlashcardStyles.tsx`；`jp-vocab-example-sentences.ts`；`mergeJpVocabWordAfterClassNotesFetch`；规则 `.cursor/rules/jp-vocab-flashcard-examples-parity.mdc`、`jp-vocab-study-scroll-stable.mdc` |
...
|-----------|--------|------|--------|------|
| `/en-vocab` | 英语抽背-老师端 | `src/app/en-vocab/page.tsx` | `EnVocabPage variant="teacher"` | **须登录**（全页 `TeacherReviewAuth`，不对普通网友开放）；**抽查卡片**、勾选熟悉程度（**勾选后 1h 内可改用法/总体**）、共享到今日单词；学生 peek 后顶栏醒目「该学生已查看」钉到点下一个；隐藏抽查排行/手动添加 |
...
| `/en-vocab/study` | 今日英语单词 | `src/app/en-vocab/study/page.tsx` | `EnVocabStudyPageClient.tsx`（`ssr:false` 壳）→ `EnVocabStudyPage.tsx` | **管理员 / `en_vocab:study` 学生**（英语老师不可见）；点单词开详情卡；peek「查看老师正在抽查的单词」；老师端提示「该学生已查看」 |
...
| **学生已查看顶栏提示**（peek 后醒目横幅钉在卡头，一直亮到点「下一个」；闩锁勿被 poll false 冲掉） | `EnVocabPage` `studentPeekedCurrentWord`；`EnVocabTeacherQuizFlashcardModal` `__student-peek-banner`；规则 `en-vocab-student-peek-banner.mdc`；回归 `check_en_vocab_study_flashcard_parity.py` |
...
| **管理员重置清共享**（全部重置清全部 shared；今日重置清当日 shared + live；API 回 `shared_today_word_ids:[]`；客户端清空缓存锁） | `clearEnVocabSharedOnReset` / `clearJpVocabSharedOnReset`；`EnVocabPage` / `JpVocabPage` → `runReset`；规则 `.cursor/rules/vocab-reset-clears-shared.mdc`；回归 `scripts/check_vocab_reset_clears_shared.py` |
| **老师端抽查卡片**（点「抽查」/「继续抽查」或点池内词条；**近全屏网页式弹层**非新路由；桌面左：单词/释义/操作，**备注在释义窗格下方**（长文区内滚动 +「查看全部」弹窗；图片小图可点放大），右：用法与例句；**中间滚**；**「上一个/下一个」按钮窗格钉底**；窄卡片备份 `EnVocabTeacherQuizFlashcardModal.card-compact.tsx`；**模式自动随机选正序或随机**；开始前可弹操作说明；池内熟悉程度**仅能在卡片内勾选**；**有编号用法时在每条用法旁勾熟悉程度，全部勾齐才写库/共享**（`aggregateEnVocabUsageLevels` / `last_usage_levels`）；点「下一个」若用法未齐则提示框列出未勾的 `1.用法、2.用法…`（**不**滚动定位）；进行中隐藏列表；进度完成后关卡片并展示今日已抽列表；池=管理员设的 `quiz_target`（`visible_ids` 或日序 1…N，默认 20）；**学生 peek 后顶栏「该学生已查看」钉到下一个**） | `EnVocabPage.tsx` → `startTeacherQuizWithRandomMode`、`hideTeacherQuizList`、`teacherQuizLocksTable`、`recordUsageLevels`、`syncTeacherQuizLiveWord`、`studentPeekedCurrentWord`；`EnVocabTeacherQuizIntroModal.tsx`；`EnVocabTeacherQuizFlashcardModal.tsx`（`en-vocab-flashcard-page`）；备份 `….card-compact.tsx`；`JpVocabTeacherQuizFlashcardStyles.tsx`；`en-vocab-teacher-quiz.ts`；`en-vocab-teacher-quiz-storage.ts`；`en-vocab-daily-quiz-progress.ts`；`POST/GET /api/en-vocab/teacher-quiz-live`；常量 `en-vocab-page-constants.ts`；规则 `en-vocab-flashcard-page-sheet.mdc`、`en-vocab-usage-level-aggregate.mdc`、`en-vocab-study-flashcard-parity.mdc`、`en-vocab-level-lock-1h.mdc`、`en-vocab-student-peek-banner.mdc`；回归 `scripts/check_en_vocab_flashcard_notes_footer.py`、`scripts/check_en_vocab_usage_level_aggregate.py`、`scripts/check_en_vocab_review_lock.py`、`scripts/check_en_vocab_study_flashcard_parity.py` |
| **今日英语单词详情卡 / peek**（点列表单词开卡；老师新发词自动弹卡；「查看老师正在抽查的单词」写入 live peek → 老师端提示已查看；`mode="study"` 熟悉程度只展示不可改；卡内可见释义/用法例句/备注/统计） | `EnVocabStudyPage.tsx` → `openStudyFlashcard`、`peekTeacherQuizWord`、`pendingFlashcardWordIdRef`；`EnVocabTeacherQuizFlashcardModal` `mode="study"`；`POST /api/en-vocab/teacher-quiz-live`；规则 `.cursor/rules/en-vocab-study-flashcard-parity.mdc`；回归 `python3 scripts/check_en_vocab_study_flashcard_parity.py` |
...
| **词表「用法 / 例句」合并查看**（单列「查看 (N)」弹窗；**禁止**拆成两列或行内展开；卡片同配对展示：`1.用法`→例句→译文，阿拉伯数字） | `EnVocabUsageExamplesCell.tsx` → `EnVocabUsageViewModal` + `EnVocabUsageExamplesPairedContent`；配对 `en-vocab-usage-examples-display.ts`；列宽 `.jp-vocab-usage-ex-col`；规则 `.cursor/rules/en-vocab-examples-view-modal.mdc` |
...
| **管理员端「更新时间」列**（最近一次 `updated_at`，北京时间紧凑显示；表头可点排序升/降；仅 `/en-vocab/admin`） | `EnVocabPage.tsx` → `jp-vocab-updated-col` + `toggleStatSort("updated")`；`en-vocab-shared.ts` → `EnVocabStatSortKey` 含 `"updated"`；`formatBeijingDateTimeCompact` |
| **音标 / 释义+词性 / 用法 / 例句补全**（顺序：音标→释义→词性→**用法→例句**；例句 `list_missing` **必须已有 usage**，一条用法造一句、用词尽量简单；可 `mode:"clear_all"` 清空例句后按用法重造；Mac 每分钟检测；**一键切换** `EN_VOCAB_FILL_LLM_BACKEND` 0=本地分阶段 Ollama / 1=线上付费 tokken 一次补齐（`en_vocab_llm_backend.py`）；本地各阶段 launchd + dirlock + ollama_slot（gemma→14b→7b）；右下角「来源」角标；**线上展示按序号把用法与例句配对**，定时任务本身不改） | API：`POST /api/en-vocab/fill-reading`、`fill-meaning`、`fill-usage`、`fill-example-sentences`（含 `clear_all`）；`src/lib/en-vocab-fill-*.ts`、`en-vocab-meaning-ai.ts`、`en-vocab-example-sentences-ai.ts`、`en-vocab-usage-ai.ts`、`en-vocab-usage-examples-display.ts`；脚本：`scripts/en-vocab-fill-*-api.py`、`en-vocab-clear-example-sentences.py`、`en-vocab-fill-stage.sh`、`setup-en-vocab-fill-mac.sh`；规则 `.cursor/rules/en-vocab-fill.mdc`；UI：`EnVocabPage` + `EnVocabUsageExamplesCell` + `EnVocabUsageViewModal` + `EnVocabTeacherQuizFlashcardModal` + `JpVocabSourceLabel` |
...
| **随手画**（图片教案批注：画笔 / **涂抹**（拖拽框选后深色盖住 + 块上标注「此内容由AI生成，经核验不准确，已涂抹」）/ 直线 / 文字 / 缩放；可下载或「保存为最新教案」覆盖本条；**仅保存成功后**「查看」页按 `updated_at` 静默换图，未保存不刷新、不整页跳动） | `JpLessonAnnotateModal.tsx`（英语 `EnLessonAnnotateModal.tsx`）；列表点「随手画」；**imageUrl 必须用** `jpVocabRefApiPath` / `enVocabRefApiPath`（勿传 `refViewUrl` 查看页 HTML）；保存 `POST /api/jp-lesson/ref/replace` → `notifyVocabRefUpdated`；查看页 `JpVocabRefViewer` + `useVocabRefLiveVersion`（`GET …/ref/[refKey]?meta=1`）；规则 `.cursor/rules/lesson-annotate-image-url.mdc`、`.cursor/rules/vocab-ref-view-live-refresh.mdc`；回归 `scripts/check_lesson_annotate_image_url.py`、`scripts/check_vocab_ref_live_refresh.py` |
...
| **状态 Tab**（学习中/未完成/已完成；**手机+桌面**点选只显示当前一类；刷新后保持，勿回到学习中；搜索时跨状态展示） | `lesson-mobile-status-filter.ts`；`JpLessonPage.tsx` / `EnLessonPage.tsx` → `mobileStatusFilter`；规则 `.cursor/rules/jp-lesson-mobile-status-tab.mdc` |
...
| **手动日程标题四选**（韩语 / 日语 / 英语按钮 + 第四格自填；不写「其他」；选预设即写入对应标题，便于科目推断） | `JpLessonManualScheduleModal.tsx` → `MANUAL_SCHEDULE_TITLE_PRESETS` |
...
| **操作按钮 3 列网格**（每行 3 个，排不下换行；桌面表格 + 手机卡片） | `AdminUsersPage.tsx` → `.admin-user-actions`；`mobile.css` |
...
| **今日有课老师账号自动启用**（北京时间 05:00；仅日语新课排课 + 手动日程；**不含英语课/英语老师**——英语老师不建登录账号；`admin` / `user1` / `test` / **永不禁用** 不受控） | `src/lib/teacher-user-schedule-enable.ts`；`POST /api/admin/teacher-user-schedule-enable`；Mac 定时 `scripts/teacher-user-schedule-enable.sh` + `setup-teacher-user-schedule-enable-mac.sh` |
