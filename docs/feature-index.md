...
| **状态 Tab**（学习中/未完成/已完成；日语另有**上课中**=学习中∩李老师快捷；**手机+桌面**点选只显示当前一类；刷新后保持，勿回到学习中；搜索时跨状态展示） | `lesson-mobile-status-filter.ts`（日语 `readStoredJpLessonListFilter` 含 `in_class`）；`JpLessonPage.tsx` / `jp-lesson-page/JpLessonPageSections.tsx`；`jpLessonAssignedToInClassTeacher`；规则 `.cursor/rules/jp-lesson-mobile-status-tab.mdc`；回归 `scripts/check_jp_lesson_in_class_tab.py` |
