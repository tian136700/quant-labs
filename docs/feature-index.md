...
| **导出 Word**（全部数据 / 今日未掌握；词条分块排版、备注图片嵌入 Word 三列展示；图片 ≥4 张的词条独占一页；不含熟悉程度与巧记） | `JpVocabPage.tsx` → `runExport`；`JpVocabExportChoiceModal.tsx`；`jp-vocab-export.ts` → `exportJpVocabToWord` |
...
| 导航菜单文案 | `src/i18n/messages.ts` → `nav.jpVocab`、`nav.jpVocabStudy`、`nav.jpVocabReview` |
