...
| **久未复习抬升权重**（`final_score = priority + days × weight`；**从未抽查不算分、日序默认置顶**；已抽查日序主依据改为 **SRS 到期日**，`final_score` 作同档打平；默认权重 0.1；管理员可改；次日凌晨或今日重置重排） | `JpVocabQuizTimeWeightAdmin.tsx`；`jpVocabAppliesFinalQuizScore` / `jpVocabFinalQuizScoreOrNull`；`sortJpVocabWordsForDailyOrder`；`jp-vocab-srs.ts`；规则 `.cursor/rules/jp-vocab-quiz-time-weight.mdc`、`.cursor/rules/jp-vocab-srs.mdc`；回归 `scripts/check_jp_vocab_quiz_score.py`、`scripts/check_jp_vocab_srs.py` |
...
| **释义补全**（`list_missing` / `apply` / **`clear_all`**；多义用「；」最多 3、常用在前；缺释义时若同时缺**词性/例句**则同一次 Cloud 一并补（`need_pos`/`need_examples`，例句只要常用用法）；**无本机 Ollama 定时**；**tokken Anthropic** 与英语线上同套，硬限流 ≥1s/条、前一任务未完则等待；新课**不同步**释义） | `POST /api/jp-vocab/fill-meaning`；`jp-vocab-fill-meaning.ts` / `jp-vocab-meaning-ai.ts`；`meaning_source`；`scripts/jp-vocab-fill-meaning-api.py` + `scripts/lib/paid_anthropic_client.py`；规则 `.cursor/rules/jp-vocab-fill-meaning.mdc` |
| **例句 / 造句补全**（`list_missing` 拉缺例句+`prompt`；`apply` 写回并传 `source`；老师卡片显示「例句来源」；人手改记为「手动」；**Mac 定时静默=今日最后抽查后再等 1h**，见 `fill-schedule-gate`；**存库仍 `漢字(かな)`，页面用 `JpVocabFuriganaText`**；**な形容词「〜だ」用词干造句**见 `jp-vocab-na-adj.ts`；**语法须先有 `usage`，例句与用法 1:1**） | `POST /api/jp-vocab/fill-example-sentences`；`POST /api/jp-vocab/fill-schedule-gate`；STT `com.stt.jp_vocab_remote_fill_examples`；规则 compose + fill-example-sentences |
| **语法用法补全**（仅 `kind=grammar`；编号中文用法 N5～N2 常用度降序；卡片「用法 / 例句」配对展示；tokken 付费串行 ≥1s/条；可 `clear_grammar_examples` 只清语法例句） | `POST /api/jp-vocab/fill-usage`；`jp-vocab-usage-ai.ts` / `jp-vocab-fill-usage.ts`；`JpVocabUsageExamplesPairedContent`；`scripts/jp-vocab-fill-grammar-usage-examples-api.py`；规则 `.cursor/rules/jp-vocab-grammar-usage.mdc` |
...
| `/admin` | 后台管理（访问与操作日志、用户反馈；**IP 可点复制** + CopyToast） | `admin/page.tsx` | `AdminDashboardPage.tsx` → `AdminIpCopyButton`；`copyTextToClipboard` + `CopyToast` |
...
| **历史登录 IP**（IP 下列「查看历史登录IP」；每次登录写入 `etr_user_login_history`；弹窗按时间新→旧列出；**归属地精确到区县**，[ip9.com.cn](https://ip9.com.cn/get?ip=) + D1 缓存 `etr_ip_geo_cache`；**串行节流**禁止并行；**Mac 每 30s 回填 1 个唯一 IP**） | `AdminUserLoginHistoryModal`；`GET …/login-history`；`GET …/ip-geo`；`POST …/ip-geo/backfill`（`status`/`step`/`requeue`）；`ip_geo_backfill.ts`；Mac `scripts/setup-login-ip-geo-backfill-mac.sh`；规则 `.cursor/rules/admin-users-ip9-geo-throttle.mdc` |
