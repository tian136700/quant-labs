...
| **历史登录 IP**（IP 下列「查看历史登录IP」；每次登录写入 `etr_user_login_history`；弹窗按时间新→旧列出） | `AdminUserIpDisplay` → `onViewHistory`；`AdminUserLoginHistoryModal`；`GET /api/admin/users/login-history`；`etr-auth-db/login_history.ts`（登录时 `recordUserLoginHistory`） |
