...
| **例句 / 造句补全**（缺例句先内置 N5 词表，再 OpenAI/Agent；条数=`max(2,用法数)`；已有日语缺中文用 gloss 脚本补「译文：」） | `scripts/jp-vocab-fill-example-sentences-ai.py`；`scripts/jp-vocab-fill-example-gloss.py`；`POST /api/jp-vocab/fill-example-sentences`；`jp-vocab-example-sentences-ai.ts`；规则 `.cursor/rules/jp-vocab-example-sentences-compose.mdc`（alwaysApply 造句规则）、`jp-vocab-fill-example-sentences.mdc`（技术入口） |
...
| **开课前 10 分钟 Bark 推送**（Mac launchd 每分钟拉 `schedule-events`；与网站日程同源；每节课只推一次） | `scripts/schedule-class-bark-remind.py`；安装 `bash scripts/setup-schedule-class-bark-remind-mac.sh`；配置 `~/.config/info-quests/schedule-class-bark-remind.env`；Bark key 复用 `.env.deploy.local`；规则 `.cursor/rules/bark-deploy-failure-notify.mdc` |
