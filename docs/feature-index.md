...
| **删除新课**（有编辑权限；二次确认；已完成先从抽问卸词；清笔记/老师/上课时间；无其它课引用时删教案 ref） | `POST /api/jp-lesson` `action: "delete"`；`jp-lesson-db-delete.ts` → `deleteJpLesson`；`useJpLessonPageActions` → `deleteLesson`；`JpLessonStatusTable` 操作列「删除」；规则 `.cursor/rules/delete-confirm.mdc` |
