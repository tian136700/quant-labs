...
| **例句 / 造句补全**（缺例句先 `list_missing` 拉词条+`prompt`/`upload_spec`，本地模型生成后再 `apply` 写回；条数=`max(2,用法数)`；不合规格式拒写） | `scripts/jp-vocab-fill-example-sentences-api.py`；`scripts/jp-vocab-fill-example-sentences-ai.py`；`POST /api/jp-vocab/fill-example-sentences`；`jp-vocab-example-sentences-ai.ts`；规则 `.cursor/rules/jp-vocab-example-sentences-compose.mdc`、`jp-vocab-fill-example-sentences.mdc` |
