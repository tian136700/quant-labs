...
| **相关构词**（单词卡例句后：含本词汉字的简单词助记，如 口→入口；连浊算同一读音族；AI 统一补全顺带写入；仅单词；临时单汉字回填 `jp-vocab-fill-related-compounds-online` 每分钟 1 条，维护中心「词条补全·日语」可见） | `related_compounds` / `related_compounds_source`；`JpVocabRelatedCompoundsSection`；`jp-vocab-related-compounds.ts` / `jp-vocab-related-compounds-fill.ts`；`scripts/setup-jp-vocab-fill-related-compounds-online-mac.sh`；规则 `.cursor/rules/jp-vocab-related-compounds.mdc`；回归 `scripts/check_jp_vocab_related_compounds.py` |
...
| **1102：今日日语单词整页「rendering the page」**（HTML 走 OpenNext 冷/忙 isolate；≠备注贴图；study 壳须 `force-static`；进页 HTML 不写 worker 流量表） | `jp-vocab/study/page.tsx`；`PAGE_HTML_TRAFFIC_SKIP`（`worker-traffic-path.ts`）；规则 `vocab-study-page-html-1102.mdc`；回归 `scripts/check_jp_vocab_study_shared_1102.py` |
