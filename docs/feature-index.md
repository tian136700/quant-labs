...
| **学生端轻量轮询 shared**（老师勾选后跨设备约 ≤15s 弹卡；同浏览器靠 BroadcastChannel 立刻；凌晨/test 降频） | `useVocabStudySharedPoll.ts`；`JP_VOCAB_STUDY_POLL_MS`；规则 `shared-list-no-notes-blob.mdc`；回归 `scripts/check_vocab_study_shared_poll.py` |
