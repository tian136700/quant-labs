...
| **音标 / 释义+词性 / 用法 / 例句补全**（顺序：音标→释义→词性→**用法→例句**；用法每条带 **出现频次 [1]～[10]**，卡片/弹窗显示「出现频次 N」；例句 `list_missing` **必须已有 usage**，一条用法造一句、用词尽量简单；可 `mode:"clear_all"` 清空例句后按用法重造；Mac 每分钟检测；**一键切换** `EN_VOCAB_FILL_LLM_BACKEND` 0=本地分阶段 Ollama / 1=线上付费 tokken 一次补齐（`en_vocab_llm_backend.py`）；本地各阶段 launchd + dirlock + ollama_slot（gemma→14b→7b）；右下角「来源」角标；**线上展示按序号把用法与例句配对**，定时任务本身不改） | API：`POST /api/en-vocab/fill-reading`、`fill-meaning`、`fill-usage`、`fill-example-sentences`（含 `clear_all`）；`src/lib/en-vocab-fill-*.ts`、`en-vocab-meaning-ai.ts`、`en-vocab-example-sentences-ai.ts`、`en-vocab-usage-ai.ts`（频次）、`en-vocab-usage-examples-display.ts`；脚本：`scripts/en-vocab-fill-*-api.py`、`en-vocab-clear-example-sentences.py`、`en-vocab-fill-stage.sh`、`setup-en-vocab-fill-mac.sh`；规则 `.cursor/rules/en-vocab-fill.mdc`；UI：`EnVocabPage` + `EnVocabUsageExamplesCell` + `EnVocabUsageViewModal` + `EnVocabTeacherQuizFlashcardModal` / `EnVocabUsageExamplesPairedContent` + `JpVocabSourceLabel`；回归 `scripts/check_en_vocab_usage_frequency.py` |
