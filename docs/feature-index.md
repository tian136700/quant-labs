...
| **随手画**（图片教案批注：画笔 / **涂抹**（拖拽框选后深色盖住 + 块上标注「此内容由AI生成，经核验不准确，已涂抹」）/ 直线 / 文字 / 缩放；可下载或「保存为最新教案」覆盖本条） | `JpLessonAnnotateModal.tsx`（英语 `EnLessonAnnotateModal.tsx`）；列表点「随手画」；**imageUrl 必须用** `jpVocabRefApiPath` / `enVocabRefApiPath`（勿传 `refViewUrl` 查看页 HTML）；保存 `POST /api/jp-lesson/ref/replace`；规则 `.cursor/rules/lesson-annotate-image-url.mdc`；回归 `scripts/check_lesson_annotate_image_url.py` |
...
| **随手画 / 涂抹**（与日语同款；`EnLessonAnnotateModal`；imageUrl 用 `enVocabRefApiPath`） | 见日语新课「随手画」；规则 `lesson-annotate-image-url.mdc` |
...
| 站点导航（**管理员**：日语/英语/韩语二级菜单，「日语」组最左；**老师等非管理员**：一级扁平链接，无语言下拉；其余按频次；溢出→「更多」） | `src/hooks/useSiteNavSplit.ts`（`useLangGroups: isAdmin`）、`src/lib/site-nav-config.ts`（`NAV_LANG_GROUPS` / `PINNED_PRIMARY_NAV_ID=langJp`）、`src/lib/site-nav-groups.ts`、`src/hooks/useSiteNavItems.ts`、`src/components/SiteNav.tsx`、`src/components/NavDrawer.tsx`、`src/components/AppShell.tsx`；规则 `.cursor/rules/site-nav-pin-freq.mdc`；回归 `scripts/check_site_nav_more_visible.py` |
