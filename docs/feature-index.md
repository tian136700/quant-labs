...
| 手动添加 / 编辑词条 | `JpVocabManualAddModal.tsx`、`JpVocabEditModal.tsx`（含**巧记**字段，仅管理员；**备注可多图上传/粘贴**，展示与 `JpClassNotesEditModal` 同：居中、去重）；`/api/jp-vocab/add`、`/edit`；`jp_vocab_word.mnemonic` |
...
| 统一日程（日语/英语/手动；**「学习中」+「已完成」进日程**，未上课不同步；上完不消失；**同老师同时间单词+语法网页合成一条**） | `JpLessonSchedulePage.tsx` → `buildLessonEventDedupKey`；`jp-lesson-shared.ts` / `en-lesson-shared.ts` → `build*LessonScheduleEvents` / `*LessonProgressAppearsOnSchedule`；`jp-lesson-manual-schedule.ts` → `LessonScheduleSubject` |
