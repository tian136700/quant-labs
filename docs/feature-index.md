...
| **手动日程关联教材**（新增/编辑可选 0～2 本；标题「日语」默认日语新课，「英语」英语新课；详情侧栏可点开笔记） | `linked_lessons`；`jp-lesson-manual-schedule-linked.ts`；`JpLessonManualScheduleLessonPicker`；规则 `manual-schedule-linked-lessons.mdc`；回归 `scripts/check_manual_schedule_linked_lessons.py` |
