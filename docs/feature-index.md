...
| `/admin/worker-1102` | **1102 诊断看板**（定位 Error 1102 单次资源顶满：备注体积/贴图提示、今日共享列表载荷、热路径慢≥2s·大≥80KB·5xx 聚合、相关流量交叉、防护清单、**复制诊断报告**；真正 1102 杀进程记不到那一次） | `admin/worker-1102/page.tsx`、`zh/admin/worker-1102/page.tsx` | `AdminWorker1102Page.tsx` → `AdminWorker1102Panel`；`worker-1102-db.ts` / `worker-1102-observe.ts` / `worker-1102-report.ts`；`GET /api/analytics/worker-1102`；热路径 `shared`/`class-notes` 观察；规则 `.cursor/rules/worker-1102-diagnose-board.mdc`；回归 `scripts/check_worker_1102_board.py` |
