...
| **历史登录 IP**（IP 下列「查看历史登录IP」；每次登录写入 `etr_user_login_history`；弹窗按时间新→旧列出；**归属地精确到区县**，[ip9.com.cn](https://ip9.com.cn/get?ip=) + D1 缓存 `etr_ip_geo_cache`；**串行节流**禁止并行） | `AdminUserIpDisplay` → `onViewHistory`；`AdminUserLoginHistoryModal`；`GET /api/admin/users/login-history`；`GET /api/admin/users/ip-geo`；`etr-auth-db/login_history.ts`、`ip_geo_cache.ts`、`ip9-geo.ts`；规则 `.cursor/rules/admin-users-ip9-geo-throttle.mdc` |
