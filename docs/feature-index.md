...
| `/jp-vocab/coach` | **课堂带读**（合并队列：「一般」「不熟悉」与未带读去重合并；**打开本页自动同步今日抽问一般/不熟悉**；已带读不拉回；**带读页可改熟悉程度/编辑词条**；备注与抽问同步；**带读卡片显示例句**；列表有例句列与带读状态；**已带读北京时间次日凌晨清空**，未带读不过期） | `src/app/jp-vocab/coach/page.tsx` | `JpVocabCoachPage.tsx` | `GET/POST /api/jp-vocab/coach`（GET 内 `syncJpVocabCoachQueueFromTodayWeak`；`update_level` / `mark_coached`） | `jp_vocab_coach_item`（`word_id` 主键 + `coached_at`）；跨日清理见 `daily-rollover` | `jp_vocab:read` 浏览；`jp_vocab:operate` 编辑/带读 |
...
| **导出到课堂带读**（今日「一般」「不熟悉」→ `/jp-vocab/coach` 合并队列；剔除已带读、与未带读去重；备注共用 `class_notes`；带读页可改熟悉程度；**打开带读页也会自动 sync**） | `JpVocabPage.tsx` → `runCoachExport`、`goToCoachPage`；`GET /api/jp-vocab/coach` → `syncJpVocabCoachQueueFromTodayWeak`；`JpVocabCoachPage.tsx` → `recordCoachLevel`；`POST /api/jp-vocab/coach` `merge_queue` / `update_level`；`jp-vocab-coach.ts` |
