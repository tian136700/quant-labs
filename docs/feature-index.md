...
| **例句 / 造句补全**（`list_missing` 拉缺例句+`prompt`；`apply` 写回并传 `source`；老师卡片显示「例句来源」；人手改记为「手动」；**Mac 定时静默=今日最后抽查后再等 1h**，见 `fill-schedule-gate`；**存库仍 `漢字(かな)`，页面用 `JpVocabFuriganaText`**） | `POST /api/jp-vocab/fill-example-sentences`；`POST /api/jp-vocab/fill-schedule-gate`；STT `com.stt.jp_vocab_remote_fill_examples`；规则 compose + fill-example-sentences |
