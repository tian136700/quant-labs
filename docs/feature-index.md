...
| **老师端正序/随机抽查卡片**（开始前弹出操作说明，可永久关闭；选定模式后即锁定列表；熟悉程度仅能在卡片内勾选；关闭卡片后点列表会重新打开卡片；会话按用户+日期+抽查数存 sessionStorage） | `JpVocabPage.tsx` → `requestTeacherQuizSession`、`finishTeacherQuiz`、`teacherQuizLocksTable`；`JpVocabTeacherQuizIntroModal.tsx`；`JpVocabTeacherQuizFlashcardModal.tsx`；`jp-vocab-teacher-quiz-storage.ts` |
