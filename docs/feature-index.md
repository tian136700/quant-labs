...
| **巧记 / 用法 / 例句字段**（管理员端表列「查看」弹窗；**编辑弹窗可填巧记、用法、例句**；巧记仅管理员；用法多行可贴图居中；备注同可贴图；例句英文行+「译文：」；人手改记为「手动」） | DB：`en_vocab_word.mnemonic` / `usage` / `usage_source` / `example_sentences` / `example_sentences_source`；`EnVocabEditModal` + `EnVocabImageNotesField` + `EnVocabClassNoteContent`；上传 `POST /api/en-vocab/class-notes/upload`；行内删除（管理员）`deleteWord` → `/api/en-vocab/delete` |
