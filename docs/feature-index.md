...
| **接序**（单词/语法：动词哪一形、一类·二类形容词、名词如何接；抽问卡在「用法/例句」下方单独模块；与用法/例句同次补全，禁止另开定时任务） | 列 `connection` / `connection_source`；`jp-vocab-connection-ai.ts`；语法走 `fill-usage`，单词走 `fill-example-sentences`；UI `JpVocabConnectionSection`；编辑 `JpVocabEditConnectionField` |
