...
| **今日英语单词详情卡 / peek**（点列表单词开卡；**老师点「下一个」同步后学生端自动弹卡**；「查看老师正在抽查的单词」写入 live peek → 老师端提示已查看；`mode="study"` 熟悉程度只展示不可改；卡内可见释义/用法例句/备注/统计） | `EnVocabStudyPage.tsx` → `openStudyFlashcard`、`peekTeacherQuizWord`、`pendingFlashcardWordIdRef`；`EnVocabTeacherQuizFlashcardModal` `mode="study"`；`POST /api/en-vocab/share`（点下一个）+ `POST /api/en-vocab/teacher-quiz-live`；规则 `jp-vocab-share-on-next.mdc`；规则 `.cursor/rules/en-vocab-study-flashcard-parity.mdc`、`en-vocab-usage-level-aggregate.mdc`；回归 `python3 scripts/check_en_vocab_study_flashcard_parity.py`、`check_en_vocab_usage_level_aggregate.py` |
