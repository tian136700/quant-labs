...
| **老师端抽查卡片**（点「抽查」/「继续抽查」或点池内词条；**近全屏网页式弹层**非新路由；桌面左：单词/释义，右：用法与例句；**备注在底栏熟悉程度/统计两框上方**；窄卡片备份 `EnVocabTeacherQuizFlashcardModal.card-compact.tsx`；**模式自动随机选正序或随机**；开始前可弹操作说明；池内熟悉程度**仅能在卡片内勾选**；**有编号用法时在每条用法旁勾熟悉程度，全部勾齐才写库/共享**（`aggregateEnVocabUsageLevels` / `last_usage_levels`）；点「下一个」若用法未齐则滚动定位到未勾条；进行中隐藏列表；进度完成后关卡片并展示今日已抽列表；无服务端 `visible_ids` 时池=当日序号 1…`EN_VOCAB_DAILY_QUIZ_TOP`） | `EnVocabPage.tsx` → `startTeacherQuizWithRandomMode`、`hideTeacherQuizList`、`teacherQuizLocksTable`、`recordUsageLevels`；`EnVocabTeacherQuizIntroModal.tsx`；`EnVocabTeacherQuizFlashcardModal.tsx`（`en-vocab-flashcard-page`）；备份 `….card-compact.tsx`；`JpVocabTeacherQuizFlashcardStyles.tsx`；`en-vocab-teacher-quiz.ts`；`en-vocab-teacher-quiz-storage.ts`；`en-vocab-daily-quiz-progress.ts`；常量 `en-vocab-page-constants.ts`；规则 `en-vocab-flashcard-page-sheet.mdc`、`en-vocab-usage-level-aggregate.mdc`；回归 `scripts/check_en_vocab_flashcard_notes_footer.py`、`scripts/check_en_vocab_usage_level_aggregate.py` |
