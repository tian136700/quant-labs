...
| **复制账号密码**（按老师身份选子域：有日语身份 → `japanese…/jp-vocab`；仅英语 → `english…/en-vocab`；仅韩语 → `korean…/ko-pron`；**勿只看主 role**；密码来自本机缓存；**李老师 / user1 等保留账号无缓存时禁止一键随机重置**，须「编辑」填写） | `AdminUsersPage.tsx` → `copyUserCredentials`；`admin-user-credentials.ts` → `formatAdminUserCredentials` + `loginLinkSiteForTeacher`；规则 `.cursor/rules/admin-user-credentials-site.mdc`、`bootstrap-account-password.mdc`；回归 `scripts/check_admin_user_credentials_site.py` |
