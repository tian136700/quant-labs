...
| **学生端老师抽查进度**（列表有 2 条、目标 10 → 显示 2/10；peek 入列表即计入；**老师 today_check 已达目标则显示已完成**，即使共享偶发少几条） | `computeJpVocabStudyPageQuizProgress`（`teacherComplete`）；`getJpVocabStudyQuizProgressTarget`；`JpVocabStudyPage.tsx`；漏共享补发 `useVocabShareBackfillOnComplete`；回归 `scripts/check_jp_vocab_study_quiz_progress.py` |
