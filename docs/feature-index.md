...
| **随手画**（图片 / **PDF 整份转竖向长图**批注：画笔 / **涂抹** / 直线 / 文字 / 缩放；打开后可下滑浏览全部页；缩放默认取 0.5～2 中位；可下载或「保存为最新教案」按页裁回 PDF；列表有「查看」；**仅保存成功后**「查看」页按 `updated_at` 静默换图） | `LessonAnnotateModal` + `openAnnotatePdfAsImageStrip` / `splitAnnotatedStripToPdfBlob`；**imageUrl 必须用** `jpVocabRefApiPath` / `enVocabRefApiPath`（勿传查看页 HTML）；PDF 传 `mediaType: "pdf"`；保存 `POST /api/jp-lesson/ref/replace`；规则 `.cursor/rules/lesson-annotate-image-url.mdc`、`.cursor/rules/vocab-ref-view-live-refresh.mdc`；回归 `scripts/check_lesson_annotate_image_url.py` |
