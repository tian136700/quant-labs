...
| **学生已查看顶栏提示**（peek →「该学生已获取该单词」；老师勾选同步 →「该单词已同步」；peek 优先；钉到点「下一个」；闩锁勿被 poll false 冲掉） | `EnVocabPage` `studentPeekedCurrentWord` + `sharedTodayWordIds`；`EnVocabFlashcardPageHeader` `__student-peek-banner`；规则 `en-vocab-student-peek-banner.mdc`；回归 `check_en_vocab_study_flashcard_parity.py` |
