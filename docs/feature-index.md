...
| **词表「用法 / 例句」合并查看**（单列「查看 (N)」弹窗；**禁止**拆成两列或行内展开；卡片同配对展示：用法一→例句→译文） | `EnVocabUsageExamplesCell.tsx` → `EnVocabUsageViewModal` + `EnVocabUsageExamplesPairedContent`；配对 `en-vocab-usage-examples-display.ts`；列宽 `.jp-vocab-usage-ex-col`；规则 `.cursor/rules/en-vocab-examples-view-modal.mdc` |
