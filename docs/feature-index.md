...
| **韩语发音复习**（列表 + 开始/继续/清除进度；卡：未揭示只露字母；「显示读音」→罗马音+自动发音；**每次队列乱序**；**复习中 overlay 不透明 + 列表藏罗马音/发音键防剧透**；无熟悉程度） | `KoPronReviewPage`；`KoPronReviewFlashcardModal`；`ko-pron-review-session.ts` → `buildKoPronReviewSession`；`/api/ko-pron/review`；`ko_pron_review_done`；规则 `.cursor/rules/ko-pron-review-no-spoiler.mdc` |
