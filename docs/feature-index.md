...
| **韩语发音复习**（**重新开始复习** / **继续复习**（误关从断点接着，如第 4 张）；清除本轮进度；卡：显示读音→熟悉/不熟悉；列表：熟悉/不熟悉/总复习；乱序；overlay 防剧透） | `KoPronReviewPage`；`KoPronReviewFlashcardModal`；`buildKoPronReviewSession` + `read/writeKoPronReviewInterruptedSession`；`recordKoPronReviewDone`；规则 `.cursor/rules/ko-pron-review-no-spoiler.mdc` |
