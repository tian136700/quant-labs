...
| **设置上课时间 · 文字拆解**（粘贴课程签到通知 → 自动填老师+日期+时间；无该老师则保存时新建） | `LessonClassNoticePasteBox`；`lesson-class-notice-parse.ts`；`EnLessonNextClassEditModal` / `JpLessonNextClassEditModal`；规则 `.cursor/rules/lesson-class-notice-paste.mdc`；回归 `python3 scripts/check_lesson_class_notice_parse.py` |
...
| **设置上课时间 · 文字拆解**（粘贴签到通知自动填老师与时间；无老师则新建） | 同日语：`LessonClassNoticePasteBox` + `lesson-class-notice-parse.ts`；`EnLessonNextClassEditModal`；回归 `scripts/check_lesson_class_notice_parse.py` |
