...
| `/admin/worker-traffic` | **流量检测看板**（定位 Error 1027：北京日配额进度、接口 Top、用户 Top、**用户×接口**交叉、**匿名流量**、**复制诊断报告**；访问日志**不**统计 API 轮询） | `admin/worker-traffic/page.tsx`、`zh/admin/worker-traffic/page.tsx` | `AdminWorkerTrafficPage.tsx` → `AdminWorkerTrafficPanel`；`worker-traffic-db.ts`（`anonymous_hits` / `top_pairs`）/ `worker-traffic-report.ts` / `worker-traffic-record.ts`（middleware 聚合 `worker_daily_hits`）；`GET /api/analytics/traffic`；规则 `.cursor/rules/worker-api-guard-1027.mdc`；回归 `scripts/check_worker_traffic.py` |
