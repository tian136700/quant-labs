...
| **复制账号密码**（按老师身份选子域：有日语身份 → `japanese…/jp-vocab`；仅英语 → `english…/en-vocab`；仅韩语 → `korean…/ko-pron`；**勿只看主 role**；**优先本机缓存同一密码，禁止每次复制都 reset**；无缓存时普通账号 confirm 后才重置；**李老师 / user1 等保留账号无缓存禁止随机重置**，须「编辑」填写；要换密请用「更换密码」） | `useAdminUsersPageActions` → `resolvePasswordForCopy` / `copyUserCredentials`；`admin-user-credentials.ts` → `readAdminUserPassword` + `formatAdminUserCredentials` + `loginLinkSiteForTeacher`；规则 `.cursor/rules/admin-user-credentials-site.mdc`、`bootstrap-account-password.mdc`；回归 `scripts/check_admin_user_credentials_site.py`、`check_admin_users_copy_password_stable.py` |
