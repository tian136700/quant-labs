...
| `/admin` | 后台管理（**Worker 流量看板 / 定位 Error 1027**：北京日配额进度、接口 Top、用户 Top、**用户×接口**交叉、**匿名流量**、**复制诊断报告**；访问日志**不**统计 API 轮询。另有访问与操作日志、用户反馈；**IP 可点复制** + CopyToast；**全列排序**；**更新时间**；**运营商** `geo_isp`；归属地回填会刷新 `updated_at`；**登录用户下拉即筛**；**未登录访问日志只保留 10 天**；**列表分页金样**） | `admin/page.tsx` | `AdminDashboardPage.tsx` → `AdminWorkerTrafficPanel` + `AdminVisitSortTh` + `nav.admin-pagination`；`worker-traffic-db.ts`（`anonymous_hits` / `top_pairs`）/ `worker-traffic-report.ts` / `worker-traffic-record.ts`（middleware 聚合 `worker_daily_hits`）；`GET /api/analytics/traffic`；`analytics-db.ts`；`GET /api/analytics/visits`；规则 `.cursor/rules/admin-dashboard-visit-logs.mdc`、`.cursor/rules/worker-api-guard-1027.mdc`、`.cursor/rules/list-pagination-standard.mdc`；回归 `scripts/check_list_pagination_standard.py`、`scripts/check_worker_traffic.py` |
