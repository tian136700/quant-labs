...
| **老师端抽查卡片**（点「抽查」或点词条即开卡片；**模式固定随机**；今日可见池内熟悉程度**仅能在卡片内勾选**；**勾选不立即同步学生**；点「下一个」/「完成抽查」才同步（每词一次）；同步中橙色进度条，完成后再跳词；**抽查队列 = visible_ids 中未勾选词**；刷新/掉线后自动回到**第一个未勾选**词卡片；**进度已完成后关卡片并展示今日已抽列表**；卡片**右上角计时器**；预览同款 UI） | `JpVocabPage.tsx` → `recordLevel(..., "flashcard")`、`ensureWordSharedBeforeNext`；`JpVocabTeacherQuizIntroModal.tsx`；`JpVocabTeacherQuizFlashcardModal.tsx`；`useJpVocabTeacherQuizNextAdvance`；规则 `.cursor/rules/jp-vocab-teacher-quiz-pool.mdc`、`.cursor/rules/jp-vocab-share-on-next.mdc`、`.cursor/rules/vocab-teacher-quiz-random-only.mdc` |
...
| **API 合传同一课单词+语法**（一次请求 → 列表仍 **两条**：word / grammar；共享 `course_label`（如「标日初级上册第23课」）+ `course_group_id`；「教材」列展示；推荐 `word_file`+`grammar_file`；各自标已完成分别进抽问，**教材课次写入词条并在卡片备注下「课数」行展示**） | `POST /api/jp-lesson/upload-mixed`；`createJpLessonMixed`；`syncLessonToVocab` → `course_label`；`JpVocabCourseFreqMetaSection`；说明 `docs/jp-lesson-upload-mixed-api.txt`；回归 `scripts/check_jp_lesson_upload_mixed.py`、`check_jp_vocab_course_label.py`、`check_jp_vocab_frequency.py` |
...
| **释义「更多/收起」**（展开后「收起」置顶；释义用 `meanings-preview`，手机勿与学习内容预览一并 `display:none`） | `jp-lesson-page-helpers.tsx`；`mobile-jp-lesson-cards.css`；`JpLessonPageStyles.tsx` |
...
| **状态 Tab**（学习中/未完成/已完成；日语另有**上课中**=开课前 10 分钟起至本节课结束（直接读 class_schedules，不依赖「学习中」门禁；日程关联课也能进），**不限老师**；**手机+桌面**点选只显示当前一类；刷新后保持，勿回到学习中；搜索时跨状态展示） | `lesson-mobile-status-filter.ts`（日语 `readStoredJpLessonListFilter` 含 `in_class`）；`JpLessonPage.tsx` / `jp-lesson-page/JpLessonPageSections.tsx`；`isJpLessonCurrentlyInClass` + `JP_LESSON_IN_CLASS_MARK_WINDOW_MINUTES`（复用 `buildJpLessonScheduleEvents` 取开课时刻）；规则 `.cursor/rules/jp-lesson-mobile-status-tab.mdc`；回归 `scripts/check_jp_lesson_in_class_tab.py` |
...
| 统一日程（日语/英语/手动；**「学习中」+「已完成」进日程**，未上课不同步；上完不消失；**同老师同时间单词+语法网页合成一条**；**手动关联教材且同开始时间 → 不另出「手动」条**；**历史总计含日语/英语/韩语**——手动日程按标题归类） | `JpLessonSchedulePage.tsx` → `buildLessonEventDedupKey` / `findDedupedLessonEventForManualLinkedCover` / `historicalDurationTotals` + `detectScheduleTeacherSubjectFromTitle`；`jp-lesson-shared.ts` / `en-lesson-shared.ts` → `build*LessonScheduleEvents` / `*LessonProgressAppearsOnSchedule`；`jp-lesson-manual-schedule-linked.ts`；`jp-lesson-manual-schedule.ts` → `LessonScheduleSubject` |
...
| **手动日程关联教材**（新增/编辑可选 0～2 本；**编辑弹窗选教材只记本地，点保存才统一写入+同步**（避免进度条两次）；**详情右侧「关联教材」**一次保存；**「复制文字模板」**一键复制时间+教案链接发给老师；选定保存后标新课**学习中**并写入时间+老师；标题「日语」默认日语新课，「英语」英语新课；详情「教材」进教案查看页 `/jp-vocab/ref/…` 或 `/en-vocab/ref/…`，勿链笔记页） | `linked_lessons`；`JpLessonManualScheduleLessonPickModal`；`JpLessonManualScheduleLinkFromDetailModal`；`buildScheduleTeacherMessageTemplate`（`jp-lesson-schedule-teacher-message.ts`）；`syncManualScheduleLinkedLessonToLearning`（仅保存/详情关联时）；`jp-lesson-manual-schedule-linked.ts`；`JpLessonManualScheduleLessonPicker`；`JpLessonSchedulePage` → `selectedManualLinkedLessons` / `openLinkLessonPick` / `copyTeacherMessageTemplate`；规则 `manual-schedule-linked-lessons.mdc`；回归 `scripts/check_manual_schedule_linked_lessons.py` |
