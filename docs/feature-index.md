...
| **音标 / 释义+词性 / 例句 / 用法补全**（Mac 每分钟检测；各阶段独立 launchd + dirlock + ollama_slot；**一律本机 Ollama 链 gemma→14b→7b**；右下角「来源」角标） | API：`POST /api/en-vocab/fill-reading`、`fill-meaning`、`fill-example-sentences`、`fill-usage`；`src/lib/en-vocab-fill-*.ts`、`en-vocab-meaning-ai.ts`、`en-vocab-example-sentences-ai.ts`、`en-vocab-usage-ai.ts`；脚本：`scripts/en-vocab-fill-*-api.py`、`en-vocab-fill-stage.sh`、`setup-en-vocab-fill-mac.sh`；规则 `.cursor/rules/en-vocab-fill.mdc`；UI：`EnVocabPage` + `EnVocabExampleSentencesCell` + `EnVocabUsageViewModal` + `JpVocabSourceLabel` |
| **巧记 / 用法字段**（管理员端表列「查看」弹窗；编辑弹窗可填；巧记仅管理员；用法多行可贴图居中；备注同可贴图） | DB：`en_vocab_word.mnemonic` / `usage` / `usage_source`；`EnVocabEditModal` + `EnVocabImageNotesField` + `EnVocabClassNoteContent`；上传 `POST /api/en-vocab/class-notes/upload`；行内删除（管理员）`deleteWord` → `/api/en-vocab/delete` |
