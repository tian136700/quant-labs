...
| **音标 / 释义+词性 / 用法 / 例句补全**（顺序：音标→释义→词性→**用法→例句**；例句 `list_missing` **必须已有 usage**，一条用法造一句、用词尽量简单；可 `mode:"clear_all"` 清空例句后按用法重造；Mac 每分钟检测；各阶段独立 launchd + dirlock + ollama_slot；**一律本机 Ollama 链 gemma→14b→7b**；右下角「来源」角标） | API：`POST /api/en-vocab/fill-reading`、`fill-meaning`、`fill-usage`、`fill-example-sentences`（含 `clear_all`）；`src/lib/en-vocab-fill-*.ts`、`en-vocab-meaning-ai.ts`、`en-vocab-example-sentences-ai.ts`、`en-vocab-usage-ai.ts`；脚本：`scripts/en-vocab-fill-*-api.py`、`en-vocab-clear-example-sentences.py`、`en-vocab-fill-stage.sh`、`setup-en-vocab-fill-mac.sh`；规则 `.cursor/rules/en-vocab-fill.mdc`；UI：`EnVocabPage` + `EnVocabExampleSentencesCell` + `EnVocabUsageViewModal` + `JpVocabSourceLabel` |
