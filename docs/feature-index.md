...
| **日语补全 · 最近词条**（维护中心独立页签：统一补全跑过哪些词；**可配置运行间隔** 1～30 分钟，写回 launchd + `~/.config/info-quests/jp-vocab-fill.env`；摘要须写「日语统一补全定时：已启用…」勿含糊「定时状态：正在运行」，临时词性 done 须注明与统一补全无关） | UI `view-jp-fill` / `#jp-fill-interval`；`vocabFillScheduleLine`（`static/app.js`）；`jp_vocab_fill_feed.py`、`jp_vocab_fill_interval.py`；API `GET /api/jp-vocab-fill/recent`、`POST /api/jp-vocab-fill/interval`；安装 `setup-jp-vocab-fill-unified-mac.sh`；规则 `.cursor/rules/vocab-fill-schedule-status-label.mdc`；回归 `scripts/check_jp_vocab_fill_feed.py`、`check_jp_vocab_fill_unified_interval_config.py` |
