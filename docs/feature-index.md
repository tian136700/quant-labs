...
| **久未复习抬升权重**（`final_score = priority + days × weight`；默认 0.1；管理员可改 0.05/0.1/0.2/0.3；次日凌晨或今日重置重排；避免「非常熟悉」后老词永远抽不到） | `JpVocabQuizTimeWeightAdmin.tsx`；`JpVocabPage.tsx` → `setQuizTimeWeightConfig`；`POST /api/jp-vocab` `set_quiz_time_weight`；`jp-vocab-quiz-score.ts`；`get/setJpVocabQuizTimeWeight`；日序 `sortJpVocabWordsForDailyOrder`；规则 `.cursor/rules/jp-vocab-quiz-time-weight.mdc`；回归 `scripts/check_jp_vocab_quiz_score.py` |
