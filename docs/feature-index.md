...
| **老师可见池 / 隐藏已抽查 / 列表条数与进度不一致**（如设 40、已抽查 30、开隐藏后应剩 10 条却显示 40/20；从未抽查词超过剩余数时只截取剩余数；**调高目标后新增的词条勾选后仍可见**；搜索全库） | **核心** `src/lib/jp-vocab-teacher-visible.ts`（`sticky_visible_ids`、`computeJpVocabStickyVisibleIds`、`buildJpVocabQuizTargetVisibleIds`、`filterJpVocabWordsByTeacherVisibleLimit` 内 `jpVocabDailyQuizRemaining` 截断、`shouldMaterializeJpVocabTeacherVisibleLimit`）；**进度** `jp-vocab-daily-quiz-progress.ts`；**今日抽查计数** `jp-vocab-daily-check.ts`；**页面** `JpVocabPage.tsx` → `teacherVisibleWords`、`searchBaseWords`；**读库重算** `jp-vocab-db.ts` → `setJpVocabDailyQuizTarget`、`ensureJpVocabTeacherVisibleLimit()` |
...
| 管理员设抽查数量后老师列表不对 | `jp-vocab-teacher-visible.ts` → `applyJpVocabQuizTargetVisiblePlan`、`sticky_visible_ids`；`jp-vocab-db.ts` → `setJpVocabDailyQuizTarget`、`ensureJpVocabTeacherVisibleLimit` |
| 调高目标后老师勾选词条消失 / 无法发给学生 | `jp-vocab-teacher-visible.ts` → `sticky_visible_ids`、`filterJpVocabWordsByTeacherVisibleLimit`；`jp-vocab-db.ts` → `setJpVocabDailyQuizTarget` |
| 老师搜索只能搜当前可见几条 | `JpVocabPage.tsx` → `searchBaseWords`（有搜索词时搜 `displayedWords` 全库） |
