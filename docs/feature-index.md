...
| `/admin/worker-1102` | **1102 诊断看板**（定位 Error 1102：备注体积/贴图、今日共享载荷、热路径慢≥2s·大≥80KB·5xx、**客户端现场样本**（失败 URL / Ray / 耗时 / page_ok / shared_fail）、相关流量、防护清单、**复制诊断报告**；硬导航整页 1102 本页 JS 跑不到，靠软导航/API/其它标签页捕获） | `admin/worker-1102/page.tsx`、`zh/admin/worker-1102/page.tsx` | `AdminWorker1102Page` → `AdminWorker1102Panel`；`Worker1102ClientGuard`（挂 `Providers`）；`worker-1102-db.ts` / `worker-1102-client-events.ts` / `worker-1102-observe.ts` / `worker-1102-report.ts`；`GET/POST …/analytics/worker-1102`；规则 `worker-1102-diagnose-board.mdc`；回归 `scripts/check_worker_1102_board.py` |
