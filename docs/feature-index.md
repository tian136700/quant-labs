...
| **导出 Excel（复习次数）**（管理员端；单词 ID、名字、不熟悉/一般/非常熟悉次数、总共抽查次数；艾宾浩斯等算法复盘） | `JpVocabPage.tsx` → `runExportExcel`；`JpVocabExportChoiceModal.tsx` →「导出 Excel（复习次数）」；`jp-vocab-excel-export.ts` → `exportJpVocabReviewStatsToExcel`（点击时动态 `import("xlsx")`） |
