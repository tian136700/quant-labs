...
| **相关构词**（单词卡例句后：含本词汉字的简单词助记，如 口→入口；连浊算同一读音族；AI 统一补全顺带写入；仅单词） | `related_compounds` / `related_compounds_source`；`JpVocabRelatedCompoundsSection`；`jp-vocab-related-compounds.ts`；规则 `.cursor/rules/jp-vocab-related-compounds.mdc`；回归 `scripts/check_jp_vocab_related_compounds.py` |
