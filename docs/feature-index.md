...
|-----------|--------|------|--------|------|
| `/en-vocab` | 英语抽背-老师端 | `src/app/en-vocab/page.tsx` | `EnVocabPage variant="teacher"` | **抽查卡片**（正序/随机自动选）、勾选熟悉程度（**勾选后 1h 内可改，学生 peek/已共享不锁**）、共享到今日单词；隐藏抽查排行/手动添加（老师无 `en_vocab:manual_add`） |
