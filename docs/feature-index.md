...
| **状态 Tab**（学习中/未完成/已完成；日语另有**上课中**=日程上课时间窗口含当前北京时间，**不限老师**；**手机+桌面**点选只显示当前一类；刷新后保持，勿回到学习中；搜索时跨状态展示） | `lesson-mobile-status-filter.ts`（日语 `readStoredJpLessonListFilter` 含 `in_class`）；`JpLessonPage.tsx` / `jp-lesson-page/JpLessonPageSections.tsx`；`isJpLessonCurrentlyInClass`（复用 `buildJpLessonScheduleEvents`）；规则 `.cursor/rules/jp-lesson-mobile-status-tab.mdc`；回归 `scripts/check_jp_lesson_in_class_tab.py` |
