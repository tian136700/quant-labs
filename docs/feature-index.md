...
| **标已完成 → 抽问分片同步**（防 1102；大课勿单请求全量 upsert；进度只改状态 + `vocab_sync`；客户端循环 `action: sync_to_vocab` 每批 8；日序仅 append、禁止 listJpVocabWordsForPool；无笔记跳过） | `POST /api/jp-lesson`；`jp-lesson-vocab-sync.ts`；`runJpLessonVocabSyncChunks`；规则 `.cursor/rules/jp-lesson-complete-vocab-sync-chunked.mdc`；回归 `scripts/check_jp_lesson_vocab_sync_chunked.py` |
