...
| **Telegram 自然语言录入日程**（父项目 bot；例句「今天下午4点韩语课…老师机构老师」；缺老师按科目新建，不建登录账号；Bearer=`JP_REVIEW_UPLOAD_TOKEN`） | 父：`lib/bots/schedule_chat_command.py` + `telegram_bot.py` intent `schedule_chat`；Cloud：`POST /api/admin/schedule-chat-ingest` → `schedule-chat-ingest.ts`；鉴权 `admin-or-upload-auth.ts`（手动日程 / 老师 list·create 同开 Bearer） |
